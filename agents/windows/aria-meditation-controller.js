...
async function readCommands(offset = 0) {
  ...
  const segment = text.slice(safeOffset), entries = [];
  let cursor = safeOffset;
  for (const raw of segment.split(/\r?\n/)) {
    const line = raw.trim(); cursor += raw.length + 1;
...
async function isUserIdle() {
  const script = '[Add-Type -Name UserInput -Namespace ARIA -MemberDefinition \'[DllImport("user32.dll")] public static extern bool GetLastInputInfo(ref LASTINPUTINFO plii); [StructLayout(LayoutKind.Sequential)] public struct LASTINPUTINFO { public uint cbSize; public uint dwTime; }\' -PassThru; $i=New-Object ARIA.UserInput+LASTINPUTINFO; $i.cbSize=[Runtime.InteropServices.Marshal]::SizeOf($i); [ARIA.UserInput]::GetLastInputInfo([ref]$i)|Out-Null; [int](([Environment]::TickCount - $i.dwTime)/1000)';
  return await new Promise(resolve => {
    const p = spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', script], { windowsHide: true });
...
async function requestTick({ state, reason, tick }) {
  ...
  try {
    const response = await fetch(`${GATEWAY_URL.replace(/\/$/, '')}/v1/meditation/tick`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${DEVICE_TOKEN}`, 'x-aria-device-id': DEVICE_ID }, body: JSON.stringify({ device_id: DEVICE_ID, session_id: state.session_id, reason, tick }), signal: controller.signal });
    const text = await response.text(); let body; try { body = text ? JSON.parse(text) : {}; } catch { body = { status: 'failed', error: 'gateway_invalid_json' }; }
    if (!response.ok) return { status: 'failed', error: body?.error || `gateway_${response.status}` };
    return body;
  } catch (error) { return { status: 'failed', error: String(error?.message || error) }; }
  finally { clearTimeout(timer); }
}
async function checkpoint(record) {
  const p = path.join(MED_ROOT, 'checkpoint.json'); await fsp.mkdir(path.dirname(p), { recursive: true });
  const normalized = { version: 'aria-meditation-checkpoint-v1', recorded_at: new Date().toISOString(), ...record };
  await fsp.writeFile(p, JSON.stringify(normalized, null, 2), 'utf8'); return normalized;
}
function startControlServer(controller) {
  ...
  server.on('error', error => appendLog(`CONTROL_SERVER_ERROR ${error.message}`).catch(() => {}));
  server.listen(CONTROL_PORT, '127.0.0.1', () => appendLog(`CONTROL_SERVER online=http://127.0.0.1:${CONTROL_PORT}`).catch(() => {}));
  return server;
}
function createWindowsMeditationController() {
  const store = createFileStateStore({ root_dir: MED_ROOT });
  const core = createMeditationController({ stateStore: store, commandSource: { read: readCommands }, log: appendLog, requestTick, checkpoint, isUserIdle, ensureNotepad: async () => { await ensureLogFile(); return openNotepad(); }, heartbeat_ms: Number(process.env.ARIA_MEDITATION_HEARTBEAT_MS || 30_000), min_idle_seconds: Number(process.env.ARIA_MEDITATION_MIN_IDLE_SECONDS || 45) });
