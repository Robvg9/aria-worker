...
function parseSelfImprovementPayload(job, deviceId) {
  ...
  return {
    goal: payload.goal.trim().slice(0, 1000),
    category: String(payload.category || 'capability_gap'),
    risk: String(payload.risk || 'LOW').toUpperCase(),
    scope: payload.scope.slice(0, 20).map(String),
    proposed_changes: payload.proposed_changes.slice(0, 50),
...
function safeWorkspacePath(relativePath) {
  if (typeof relativePath !== 'string' || !relativePath.trim()) throw new Error('workspace_path_required');
  const normalized = relativePath.replace(/\\/g, '/').replace(/^\/+/, '');
  if (normalized.includes('..') || path.isAbsolute(normalized)) throw new Error('workspace_path_unsafe');
  const absolute = path.resolve(WORKSPACE_ROOT, normalized);
  if (!(absolute === WORKSPACE_ROOT || absolute.startsWith(WORKSPACE_ROOT + path.sep))) throw new Error('workspace_path_escape');
...
async function createWorkspace() {
  ...
  const read = async (relativePath) => {
    ...
    try { return await fs.readFile(target.absolute, 'utf8'); } catch (error) { if (error?.code === 'ENOENT') return ''; throw error; }
  };
  const apply = async (change) => {
    if (!change || String(change.risk_level || 'LOW').toUpperCase() !== 'LOW') return { status: 'blocked', reason: 'risk_not_allowed' };
    const target = safeWorkspacePath(change.path);
    await fs.mkdir(path.dirname(target.absolute), { recursive: true });
    await fs.writeFile(target.absolute, String(change.content || ''), 'utf8');
    return { status: 'succeeded', path: target.normalized, sandbox: true };
  };
...
async function testWorkspace({ scope = [] } = {}) {
  ...
  for (const relativePath of scope) {
    ...
    try {
      ...
      if (/\.(?:js|cjs|mjs)$/i.test(target.normalized)) {
        await execFileAsync(process.execPath, ['--check', target.absolute], { windowsHide: true, timeout: 30000, maxBuffer: 64 * 1024 });
      }
      results.push({ path: target.normalized, status: 'passed' });
    } catch (error) {
      results.push({ path: target.normalized, status: 'failed', error: String(error?.message || error) });
    }
  }
  return { status: results.every((item) => item.status === 'passed') ? 'succeeded' : 'failed', results, scope: results.map((item) => item.path) };
}

function buildSnapshot() {
  return async (include = []) => ({
    identity: 'ARIA',
    version: 'windows-local-agent-v2',
    capabilities: ['self_improvement_sandbox', 'filesystem_stage', 'node_syntax_test'],
    tools: ['node', 'self-improvement-coordinator-v1'],
    connectors: ['aria-device-gateway'],
    providers: [],
    health: { status: 'ready' },
    tests: { status: 'available', runner: 'node --check' },
    git: { branch: 'self-improvement-sandbox', isolated: true },
    inspected: Array.isArray(include) ? include : []
  });
}
...
async function writeEvidence(evidence) {
  ...
  return { status: 'succeeded', path: EVIDENCE_PATH };
}

async function executeSelfImprovementJob(job, deviceId) {
  const signal = parseSelfImprovementPayload(job, deviceId);
  const workspace = await createWorkspace();
  const coordinator = createSelfImprovementCoordinatorV1({
    snapshot: buildSnapshot(),
    workspace,
    testRunner: testWorkspace,
    writer: async (payload) => writeEvidence({
      recorded_at: new Date().toISOString(),
      mission_id: signal.mission_id,
      step_id: signal.step_id,
      coordinator: 'self-improvement-coordinator-v1',
      objective: signal.goal,
      payload
    }),
    policy: { max_risk: 'low', protected_paths: ['main', 'master'] },
    selfModel: { identity: 'ARIA', canonicalEntrypoint: 'aria-canonical-runtime-v1', softwareVersion: 'windows-local-agent-v2', capabilities: ['self_improvement_sandbox'] }
  });
  const result = await coordinator.engine.run(signal);
  return {
    status: result.status === 'completed' && result.stop_reason === 'verified' ? 'succeeded' : result.status === 'blocked' ? 'blocked' : 'failed',
    exit_code: result.status === 'completed' ? 0 : 1,
    stdout: '',
    stderr: result.status === 'completed' ? '' : String(result.stop_reason || 'self_improvement_failed'),
    coordinator_status: result.status,
    stop_reason: result.stop_reason || null,
    version: result.version || null,
    evidence_hash: result.evidence_hash || null,
...
module.exports = Object.freeze({
  WORKSPACE_ROOT,
  parseSelfImprovementPayload,
  executeSelfImprovementJob
});
