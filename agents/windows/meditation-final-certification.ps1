...
function Read-JsonFile([string]$Path){try{if(Test-Path $Path){$raw=Get-Content -Raw $Path -ErrorAction Stop;if($raw -and $raw.Trim()){return($raw|ConvertFrom-Json -ErrorAction Stop)}}}catch{};return $null}
function Get-MaxMeditationTick{if(-not(Test-Path $MeditationLogPath)){return -1};$ticks=@();foreach($line in(Get-Content $MeditationLogPath -Tail 400 -ErrorAction SilentlyContinue)){if($line -match 'IDLE tick=(\d+)'){$ticks += [int]$Matches[1]}};if($ticks.Count -eq 0){return -1};return($ticks|Measure-Object -Maximum).Maximum}
function Test-AgentAliveByPid([int]$ProcessId){if($ProcessId -le 0){return $false};try{Get-Process -Id $ProcessId -ErrorAction Stop|Out-Null;return $true}catch{return $false}}
function Resolve-NodePath{try{if(Test-Path $ConfigPath){$cfg=Get-Content -Raw $ConfigPath|ConvertFrom-Json;if($cfg.node_path -and(Test-Path([string]$cfg.node_path))){return[string]$cfg.node_path}}}catch{};$cmd=Get-Command node -ErrorAction SilentlyContinue;if($cmd){return$cmd.Source};$fallback='D:\Databank\node\node.exe';if(Test-Path $fallback){return$fallback};throw 'Node.js executable not found'}
...
if(-not[string]::IsNullOrWhiteSpace($env:ARIA_EXPECTED_SHA)){Set-Content -Path $RuntimeShaPath -Value $env:ARIA_EXPECTED_SHA -Encoding ASCII -Force}
$watchdog=$null
for($i=0;$i-lt 60;$i++){$watchdog=Read-JsonFile $StatusPath;if($watchdog -and $watchdog.state -eq 'agent_running'){break};Start-Sleep -Seconds 1}
...
$new=$null
for($i=0;$i-lt 90;$i++){$candidate=Read-JsonFile $StatusPath;if($candidate -and $candidate.state-eq'agent_running' -and -not[string]::IsNullOrWhiteSpace([string]$candidate.agent_pid) -and[string]$candidate.agent_pid-ne$beforePid){$new=$candidate;Write-Host("RECOVERY_PID_CHANGE t="+$i+"s old="+$beforePid+" new="+$new.agent_pid);break};if(($i%15)-eq 14){$st=if($candidate){$candidate.state}else{'null'};$ap=if($candidate){$candidate.agent_pid}else{''};Write-Host("RECOVERY_WAIT t="+($i+1)+"s state="+$st+" agent_pid="+$ap)};Start-Sleep -Seconds 1}
...
for($i=0;$i-lt 90;$i++){
  ...
  if($latest-and$latest.state-eq'agent_running'-and-not[string]::IsNullOrWhiteSpace([string]$latest.agent_pid)){$new=$latest;$recoveredPid=[int]$latest.agent_pid}
  if(Test-AgentAliveByPid $recoveredPid){$livePid=$recoveredPid;$recoveredState=Read-JsonFile $MeditationStatePath;if($recoveredState-and$recoveredState.version-eq'aria-meditation-ia-v1'-and$recoveredState.mode-eq'active'){Write-Host("RECOVERY_STABLE_ACTIVE t="+$i+"s pid="+$livePid+" tick="+$recoveredState.tick_count);break};Write-Host("RECOVERY_PID_LIVE t="+$i+"s pid="+$livePid)}else{
    Write-Host("RECOVERY_PID_NOT_LIVE t="+$i+"s pid="+$recoveredPid)
  }
  if(($i%15)-eq 14){Write-Host("RECOVERY_STABILIZE_WAIT t="+($i+1)+"s current_pid="+$recoveredPid)}
  Start-Sleep -Seconds 1
}
# Re-read after the stabilization window so a PID rollover at the sampling boundary is not misclassified.
$latest=Read-JsonFile $StatusPath
if($latest-and$latest.state-eq'agent_running'-and-not[string]::IsNullOrWhiteSpace([string]$latest.agent_pid)){$new=$latest;$recoveredPid=[int]$latest.agent_pid}
if(Test-AgentAliveByPid $recoveredPid){$livePid=$recoveredPid}
Assert-True($new-and$new.state-eq'agent_running')'Watchdog recovery ended without an agent_running state'
Assert-True([string]$new.agent_pid-ne$beforePid)'No post-recovery agent pid remained distinct from the original'
Assert-True($livePid-ne$null)"No live recovered Agent process after watchdog recovery; last status pid=$recoveredPid"
$recoveredState=Read-JsonFile $MeditationStatePath
$logTail=(Get-Content $MeditationLogPath -Tail 250 -ErrorAction SilentlyContinue)-join "`n"
...
if($agents.Count-gt 0){Assert-True($agents.Count-eq 1) "Expected one ARIA agent process by CommandLine, found $($agents.Count)";Assert-True([int]$agents[0].ProcessId-eq$livePid) "CommandLine agent pid $($agents[0].ProcessId) != live status pid $livePid";Write-Host 'SINGLE_AGENT_INSTANCE=PASS via=commandline'}else{Assert-True(Test-AgentAliveByPid $livePid) "Expected current Agent pid $livePid alive (CommandLine not visible to runner)";Write-Host 'SINGLE_AGENT_INSTANCE=PASS via=status_pid'}
Write-Host "ARIA_AGENT_RECOVERY=$beforePid->$livePid"
