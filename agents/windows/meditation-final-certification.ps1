...
function Sync-RuntimeTree([string]$SourceDir,[string]$DestDir){
...
foreach ($dirName in @('autonomy','self-development','self-model')) {
  ...
  Write-Host ("RUNTIME_TREE_SYNC=" + $dirName)
  Sync-RuntimeTree -SourceDir $sourceDir -DestDir $destDir
}
...
for ($i = 0; $i -lt 90; $i++) {
  ...
  if (Test-AgentAliveByPid $recoveredPid) {
    ...
    if ($recoveredState -and $recoveredState.version -eq 'aria-meditation-ia-v1' -and $recoveredState.mode -eq 'active') {
      Write-Host ("RECOVERY_STABLE_ACTIVE t=" + $i + "s pid=" + $livePid + " tick=" + $recoveredState.tick_count)
      break
    }
    Write-Host ("RECOVERY_PID_LIVE t=" + $i + "s pid=" + $livePid)
  } else {
...
Assert-True ($null -ne $livePid) ("No live recovered Agent process after watchdog recovery; last status pid=" + $recoveredPid)
$recoveredState = Read-JsonFile $MeditationStatePath
...
if ($agents.Count -gt 0) {
  ...
  Assert-True ([int]$agents[0].ProcessId -eq $livePid) ("CommandLine agent pid " + $agents[0].ProcessId + " != live status pid " + $livePid)
  Write-Host 'SINGLE_AGENT_INSTANCE=PASS via=commandline'
} else {
  Assert-True (Test-AgentAliveByPid $livePid) ("Expected current Agent pid " + $livePid + " alive (CommandLine not visible to runner)")
