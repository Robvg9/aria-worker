...
function Get-ActivityLog { try { Invoke-RestMethod -Method Get -Uri "$base/log" -TimeoutSec 3 } catch { $null } }
function Invoke-Aria([string]$path) {
    try {
        $result = Invoke-RestMethod -Method Post -Uri "$base$path" -TimeoutSec 12
        Refresh-Ui
        return $result
    } catch {
...
function Refresh-Ui {
    try {
        ...
        $a=Get-ActivityLog;if($null -ne $a -and $a.content){$lines=([string]$a.content)-split "`r?`n"|Where-Object{$_}|Select-Object -Last 22;$log.Text=$lines -join [Environment]::NewLine}
    } catch { }
}
