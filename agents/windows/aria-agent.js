...
async function api(path, options = {}) {
  ...
  for (let attempt = 0; attempt <= GATEWAY_RETRIES; attempt += 1) {
    ...
    try {
      ...
      const retryable = response.status === 401 || response.status === 408 || response.status === 429 || response.status >= 500; if (!retryable || attempt >= GATEWAY_RETRIES) throw error;
    } catch (error) {
      lastError = error; const retryable = error?.name === 'AbortError' || !Number.isInteger(error?.status) || error.status === 401 || error.status === 408 || error.status === 429 || error.status >= 500; if (!retryable || attempt >= GATEWAY_RETRIES) throw error;
...
async function callOllama({ prompt, model, timeout_ms }) {
  ...
  try {
    ...
    let body; try { body = JSON.parse(text); } catch (_) { throw new Error('ollama_invalid_json'); } if (typeof body.response !== 'string') throw new Error('ollama_response_missing');
...
async function executeShellJob(job) {
  ...
  log(`JOB RECEIVED id=${job.job_id} operation=${SHELL_OPERATION} cwd=${JSON.stringify(payload.cwd)}`); await api(`/v1/jobs/${encodeURIComponent(job.job_id)}/start`, { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID }) });
  ...
  log(`JOB RESULT id=${job.job_id} status=${result.status} exit_code=${result.exit_code} duration_ms=${result.duration_ms}`); await api(`/v1/jobs/${encodeURIComponent(job.job_id)}/result`, { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID, result }) }); log(`JOB ACK id=${job.job_id} status=${result.status}`);
}
...
async function claimAndExecute() {
  try { const body = await api('/v1/jobs/claim', { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID }) }); if (!body?.job) return; const job = body.job; if (job.device_id !== DEVICE_ID) { await rejectClaimedJob(job, 'device_id_mismatch', job.operation); return; } if (job.operation === SHELL_OPERATION) return executeShellJob(job); if (job.operation === OLLAMA_OPERATION) return executeOllamaJob(job); if (job.operation === COMPUTER_OPERATION) return executeComputerUseJob(job); await rejectClaimedJob(job, `unsupported_operation:${String(job.operation || '')}`, job.operation); }
  catch (error) { console.error(`[job] ${error.message}`); }
}
let stopping = false; process.on('SIGTERM', () => { stopping = true; log('STOP requested'); }); process.on('SIGINT', () => { stopping = true; log('STOP requested'); });
(async () => { log(`START device=${DEVICE_ID} platform=windows-local node=${process.version}`); try { await enroll(); } catch (error) { console.error(`[enroll] ${error.message}`); } await heartbeat(); setInterval(heartbeat, HEARTBEAT_MS); while (!stopping) { await claimAndExecute(); await sleep(POLL_MS); } })().catch(error => { console.error(error); process.exit(1); });
