...
function normalizeQwenResponse(value) { if (typeof value !== 'string') return ''; return value.replace(/<think>[\s\S]*?<\/think>/gi, '').trim().slice(0, MAX_OUTPUT); }
...
async function api(path, options = {}) {
  ...
  for (let attempt = 0; attempt <= GATEWAY_RETRIES; attempt += 1) {
    ...
    try {
      ...
      const text = await response.text(); let body = null; try { body = text ? JSON.parse(text) : null; } catch (_) { body = { raw: text }; }
      if (response.ok) return body;
      const error = new Error(`gateway ${response.status}: ${body?.error || 'request failed'}`); error.status = response.status; lastError = error;
...
function parseQwenPayload(job) {
  ...
  let payload; try { payload = JSON.parse(job.command); } catch (_) { throw new Error('ollama_payload_invalid_json'); }
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) throw new Error('ollama_payload_invalid');
  const keys = Object.keys(payload); if (keys.some(key => !['prompt', 'model', 'timeout_ms'].includes(key))) throw new Error('ollama_payload_field_rejected');
...
async function callOllama({ prompt, model, timeout_ms }) {
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), Math.max(1_000, timeout_ms)); const started = Date.now();
  try {
    const response = await fetch(`${OLLAMA_URL}/api/generate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model, prompt, stream: false }), signal: controller.signal });
    const text = await response.text(); if (!response.ok) throw new Error(`ollama ${response.status}: ${text.slice(0, 1024)}`);
    let body; try { body = JSON.parse(text); } catch (_) { throw new Error('ollama_invalid_json'); }
  ...
  } catch (error) { const aborted = error?.name === 'AbortError'; return { status: aborted ? 'timeout' : 'failed', exit_code: null, stdout: '', stderr: redact(String(error?.message || error).slice(0, 4096)), duration_ms: Date.now() - started }; }
  finally { clearTimeout(timer); }
}
async function enroll() { const body = await api('/v1/devices/enroll', { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID, token: DEVICE_TOKEN }) }); log(`ENROLLED device=${DEVICE_ID}`); return body; }
async function heartbeat() { try { await api('/v1/devices/heartbeat', { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID, agent_type: 'windows-local', capabilities: [OLLAMA_OPERATION, SHELL_OPERATION, COMPUTER_OPERATION] }) }); log(`ONLINE device=${DEVICE_ID}`); } catch (error) { console.error(`[heartbeat] ${error.message}`); } }
async function rejectClaimedJob(job, reason, operation = job?.operation) { const result = { status: 'failed', exit_code: null, stdout: '', stderr: reason, duration_ms: 0, metadata: { agent_version: 'aria-windows-agent-v2', operation, rejected: true } }; try { await api(`/v1/jobs/${encodeURIComponent(job.job_id)}/result`, { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID, result }) }); log(`JOB REJECTED id=${job.job_id} operation=${operation} reason=${reason}`); } catch (error) { console.error(`[reject] ${error.message}`); } }
async function executeShellJob(job) {
  let payload; try { payload = parseShellJob(job, DEVICE_ID); } catch (error) { await rejectClaimedJob(job, String(error.message || 'unsupported_shell_job'), SHELL_OPERATION); return; }
  log(`JOB RECEIVED id=${job.job_id} operation=${SHELL_OPERATION} cwd=${JSON.stringify(payload.cwd)}`); await meditationController.event(`JOB RECEIVED operation=${SHELL_OPERATION}`); await api(`/v1/jobs/${encodeURIComponent(job.job_id)}/start`, { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID }) });
  const result = await executePowerShell(payload); result.stdout = redact(result.stdout); result.stderr = redact(result.stderr); result.metadata = { ...result.metadata, agent_version: 'aria-windows-agent-v2', platform: `windows/${os.release()}`, request_nonce: crypto.randomUUID(), operation: SHELL_OPERATION };
  log(`JOB RESULT id=${job.job_id} status=${result.status} exit_code=${result.exit_code} duration_ms=${result.duration_ms}`); await meditationController.event(`JOB RESULT operation=${SHELL_OPERATION} status=${result.status} exit_code=${result.exit_code}`); await api(`/v1/jobs/${encodeURIComponent(job.job_id)}/result`, { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID, result }) }); log(`JOB ACK id=${job.job_id} status=${result.status}`);
}
async function executeOllamaJob(job) {
  ...
  log(`JOB RECEIVED id=${job.job_id} operation=${OLLAMA_OPERATION}`); await meditationController.event(`JOB RECEIVED operation=${OLLAMA_OPERATION}`); await api(`/v1/jobs/${encodeURIComponent(job.job_id)}/start`, { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID }) });
...
async function executeComputerUseJob(job) {
  ...
  log(`JOB RECEIVED id=${job.job_id} operation=${COMPUTER_OPERATION} action=${payload.action}`); await meditationController.event(`JOB RECEIVED operation=${COMPUTER_OPERATION} action=${payload.action}`); await api(`/v1/jobs/${encodeURIComponent(job.job_id)}/start`, { method: 'POST', body: JSON.stringify({ device_id: DEVICE_ID }) });
...
async function claimAndExecute() {
  try {
    ...
    if (job.operation === COMPUTER_OPERATION) return executeComputerUseJob(job);
...
(async () => {
  ...
  await heartbeat(); setInterval(heartbeat, HEARTBEAT_MS);
