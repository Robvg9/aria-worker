...
-- internal enqueue contract, including the governed physical Android UI path.
-- Android autonomous jobs have a dedicated validator so the public wrapper
-- cannot regress to an old operation allow-list.
...
REVOKE ALL ON FUNCTION aria_internal.enqueue_android_ui_job(
  text,text,text,text,text,integer,jsonb,jsonb
) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION aria_internal.enqueue_android_ui_job(
  text,text,text,text,text,integer,jsonb,jsonb
) TO service_role;
