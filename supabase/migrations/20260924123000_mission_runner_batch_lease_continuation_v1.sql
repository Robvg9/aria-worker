    ...
    select m.mission_id
     ...
     where m.mission_id=p_mission_id
       and (
         ...
         or (m.status='running' and m.lease_owner is null)
         or (m.status='running' and m.lease_until is null)
         or (m.status='waiting' and coalesce(m.checkpoint->'recovery'->>'status','')='verification_pending' and (m.lease_owner is null or m.lease_until is null or m.lease_until<clock_timestamp()))
       )
       and aria_internal.aria_mission_claim_eligible(m.mission_id)
