-- Human Gate / Safe Autonomy v2
-- Purpose: action-scoped approval before protected execution, exact action binding,
-- explicit reject/cancel states, and defense-in-depth authorization verification.
...
begin
  ...
  if decision='approve' then
    ...
    if provided_hash<>'' and provided_hash <> (gate->>'action_hash') then
      raise exception 'human_gate_action_mismatch';
    end if;
    approval_token := null;
    new_status := 'rejected';
  ...
  update aria_internal.mission_state
     set status=case when decision='approve' then 'queued' else new_status end,
         next_action=case
           when decision='approve' then 'resume:human_gate_approved'
           when decision='reject' then 'terminal:human_gate_rejected'
           else 'terminal:human_gate_cancelled'
         end,
         lease_owner=null,
         lease_until=null,
         checkpoint=jsonb_set(
           ...
           '{human_gate,verified}',
           case when decision='approve' then 'false'::jsonb else 'false'::jsonb end,
           true
         ) || jsonb_build_object(
           'human_gate',
           coalesce(checkpoint->'human_gate','{}'::jsonb) || jsonb_build_object(
             ...
             'status',new_status,
             'decision',decision,
             'decision_at',clock_timestamp()::text,
  ...
  values (
    p_mission_id,
    case when decision='approve' then 'human_gate_approved'
         when decision='reject' then 'human_gate_rejected'
         else 'human_gate_cancelled' end,
    jsonb_build_object(
      'step_id',gate->>'step_id',
      'action_hash',gate->>'action_hash',
      'decision',decision,
  ...
  return jsonb_build_object(
    ...
    'step_id',gate->>'step_id',
    'decision',decision,
    'status',case when decision='approve' then 'queued' else new_status end,
    'action_hash',gate->>'action_hash',
    'approval_token_issued',decision='approve'
  );
end;
$$;
...
create or replace function aria_internal.human_gate_authorize_execution(
  p_mission_id text,
  p_step_id text,
  p_action_hash text,
...
declare
  gate jsonb;
begin
  select checkpoint->'human_gate'
    into gate
    from aria_internal.mission_state
   where mission_id=p_mission_id;

  if gate is null then return false; end if;
  if gate->>'step_id' <> p_step_id then return false; end if;
  if gate->>'action_hash' <> p_action_hash then return false; end if;
  if gate->>'status' <> 'approved' then return false; end if;
  if coalesce(gate->>'approval_token','')='' then return false; end if;
  if gate->>'approval_token' <> p_approval_token then return false; end if;
  return true;
end;
$$;

revoke all on function aria_internal.human_gate_authorize_execution(text,text,text,text) from public, anon, authenticated;
grant execute on function aria_internal.human_gate_authorize_execution(text,text,text,text) to service_role;
