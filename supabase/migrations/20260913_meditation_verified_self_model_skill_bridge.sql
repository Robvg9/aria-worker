...
begin
  ...
  if coalesce(new.metadata->>'source','') <> 'meditation-ia-v1' then
    return new;
  end if;
  ...
  values (
    ...
    jsonb_build_object('mission_id',new.mission_id,'goal',new.goal,'skill_id',v_skill_id,'skill_factory','compile_skill_for_goal','promotion_evidence_required',3)
  );
  return new;
end;
$function$;
