-- Align Human Gate rejection with existing mission_state terminal status contract.
...
begin
  ...
  if decision='approve' then
    if provided_hash='' or provided_hash <> (gate->>'action_hash') then raise exception 'human_gate_action_mismatch'; end if;
  ...
  update aria_internal.mission_state
     set status=case when decision='approve' then 'queued' when decision='reject' then 'blocked' else 'cancelled' end,
         finished_at=case when decision in ('reject','cancel') then clock_timestamp() else null end,
  ...
  return jsonb_build_object(
    'mission_id',p_mission_id,'step_id',gate->>'step_id','decision',decision,
    'status',case when decision='approve' then 'queued' when decision='reject' then 'blocked' else 'cancelled' end,
    'action_hash',gate->>'action_hash','approval_token_issued',decision='approve');
end;
$$;
