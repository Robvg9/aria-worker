-- Allow persisted continuity proof events in the canonical mission event stream.
...
alter table aria_internal.mission_events
  add constraint mission_events_event_type_check check (
    event_type = any (array[
      ...
      'self_improvement_human_gate','recovery_attempted','mission_verified','mission_dead_lettered',
      'cognitive_recall_completed','cognitive_planning_context_used','cognitive_loop_completed',
      'agent_executor_diagnostic','mission_chain_completed'
    ]::text[])
  );

comment on constraint mission_events_event_type_check on aria_internal.mission_events
is 'Canonical mission lifecycle events including durable immediate-chain evidence.';
