...
function originAllowed(req: Request) {
  ...
  return origin === "https://grok.com" || origin === "https://www.grok.com" || origin === "https://x.com" || origin === "https://www.x.com";
}
...
Deno.serve(async req => {
  ...
  } else if (![LEGACY_VERSION, "2025-06-18", "2025-03-26"].includes(requestedProtocol)) {
  ...
  const p = body.params ?? {};
  const name = typeof p.name === "string" ? p.name : headerName;
  const args = p.arguments ?? {};
  if (name === "aria_context") {
    if (typeof args.query !== "string" || !args.query.trim()) return modern ? modernReply(200, rpcError(id, -32602, "query is required")) : reply(200, rpcError(id, -32602, "query is required"));
    const out = await callFunction("aria-context-retrieval-v1", auth.token, { query: args.query.trim() });
    const response = { jsonrpc: "2.0", id, result: { content: [{ type: "text", text: JSON.stringify(out.body) }], isError: out.status >= 400 } };
    return modern ? modernReply(200, response) : reply(200, response);
  }
  if (name === "aria_memory_capture") {
    if (typeof args.message !== "string" || !args.message.trim()) return modern ? modernReply(200, rpcError(id, -32602, "message is required")) : reply(200, rpcError(id, -32602, "message is required"));
