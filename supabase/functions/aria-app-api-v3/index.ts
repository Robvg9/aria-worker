...
async function liveAssistantContext(userId:string) {
  ...
  const activeRank=(m:any)=>{const s=String(m?.status||"");return s==="running"?50:s==="waiting"?40:s==="planning"?30:s==="paused"?20:s==="queued"?10:0;};
  const active=[...owned].sort((a:any,b:any)=>activeRank(b)-activeRank(a)||new Date(String(b.updated_at||0)).getTime()-new Date(String(a.updated_at||0)).getTime())[0]??null;
