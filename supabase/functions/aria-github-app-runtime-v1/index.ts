...
function der(t:number,c:Uint8Array){const n=c.length,l=n<128?Uint8Array.of(n):(()=>{const a:number[]=[];for(let x=n;x;x>>>=8)a.unshift(x&255);return Uint8Array.of(0x80|a.length,...a)})();const o=new Uint8Array(1+l.length+c.length);o[0]=t;o.set(l,1);o.set(c,1+l.length);return o}
function cat(...xs:Uint8Array[]){const n=xs.reduce((a,b)=>a+b.length,0),o=new Uint8Array(n);let i=0;for(const x of xs){o.set(x,i);i+=x.length}return o}
function pkcs1To8(k:Uint8Array){const oid=Uint8Array.of(6,9,42,134,72,134,247,13,1,1,1);return der(48,cat(der(2,Uint8Array.of(0)),der(48,cat(oid,Uint8Array.of(5,0))),der(4,k)))}
async function appJwt(){const pk=Deno.env.get("ARIA_GITHUB_APP_PRIVATE_KEY")??"";if(!pk)throw new Error("github_app_private_key_missing");const raw=b64(pk.replace(/-----BEGIN (?:RSA )?PRIVATE KEY-----/g,"").replace(/-----END (?:RSA )?PRIVATE KEY-----/g,""));const keyDer=pk.includes("BEGIN RSA PRIVATE KEY")?pkcs1To8(raw):raw;const key=await crypto.subtle.importKey("pkcs8",keyDer.buffer,{name:"RSASSA-PKCS1-v1_5",hash:"SHA-256"},false,["sign"]);const now=Math.floor(Date.now()/1000),h=u64j({alg:"RS256",typ:"JWT"}),p=u64j({iat:now-30,exp:now+540,iss:APP_ID}),sig=new Uint8Array(await crypto.subtle.sign("RSASSA-PKCS1-v1_5",key,new TextEncoder().encode(`${h}.${p}`)));return`${h}.${p}.${u64(sig)}`}
async function gh(path:string,init:RequestInit={},token:string){const r=await fetch(API+path,{...init,headers:{accept:"application/vnd.github+json",authorization:`Bearer ${token}`,"X-GitHub-Api-Version":"2022-11-28","content-type":"application/json","user-agent":"ARIA-github-app-runtime-v1",...(init.headers||{})}});const j:any=await r.json().catch(()=>null);if(!r.ok)throw new Error(`github_${r.status}:${j?.message??"request_failed"}`);return j}
...
if(op==="tree_read"){
  const branch=String(b.branch||b.ref||b.base||"main");
  ...
  if(!sha)throw new Error("github_tree_ref_missing");
  const tree=await gh(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(rp)}/git/trees/${encodeURIComponent(sha)}?recursive=1`,{},t.token);
  return out({ok:true,status:200,executor_type:"github_app",connector_id:"github",operation:op,data:{branch,sha,truncated:tree?.truncated===true,paths:Array.isArray(tree?.tree)?tree.tree.filter((x:any)=>x?.type==="blob").map((x:any)=>String(x.path||"")).slice(0,5000):[]},installation_id:installation.id,account:installation.account});
}

if(op==="code_search"){
  const query=String(b.query||"").trim();
  if(!query)throw new Error("github_code_search_query_missing");
  const scoped=`${query} repo:${owner}/${rp}`;
...
if(op==="pr_checks"){
  ...
  return out({ok:true,status:200,executor_type:"github_app",connector_id:"github",operation:op,data:{number,head_sha:sha,total:items.length,pending:pending.length,failed:failed.length,all_passed:items.length>0&&pending.length===0&&failed.length===0,runs:items.map((x:any)=>({name:x.name,status:x.status,conclusion:x.conclusion,completed_at:x.completed_at,html_url:x.html_url}))},installation_id:installation.id,account:installation.account});
}
...
if(op==="pr_merge"){
  ...
  const pending=items.filter((x:any)=>String(x.status||"")!=="completed");
  const failed=items.filter((x:any)=>String(x.status||"")==="completed"&&!["success","neutral","skipped"].includes(String(x.conclusion||"").toLowerCase()));
  if(items.length===0)throw new Error("auto_merge_checks_missing");
...
throw new Error("operation_not_allowed")}catch(e){return out({ok:false,status:"failed",error:e instanceof Error?e.message:String(e)},200)}});
