...
const RESOURCE_METADATA = `${SUPABASE_URL}/.well-known/oauth-protected-resource/functions/v1/aria-mcp-server-grok-v2`;
const ROOT_RESOURCE_METADATA = `${SUPABASE_URL}/.well-known/oauth-protected-resource`;
...
const authChallenge = (metadata: string) => `Bearer resource_metadata="${metadata}", scope="${SCOPES}"`;
...
Deno.serve(async req => {
  ...
  if (req.method === "GET" && (u.pathname === "/.well-known/oauth-protected-resource" || u.pathname === `/.well-known/oauth-protected-resource/functions/v1/aria-mcp-server-grok-v2`)) return reply(200, resourceMetadata, { "access-control-allow-origin": "*" });
  if (req.method === "GET" || req.method === "HEAD") return reply(401, { error: "unauthorized", transport: TRANSPORT }, { "WWW-Authenticate": authChallenge(RESOURCE_METADATA) });
  ...
  if (!auth) return reply(401, { error: "unauthorized" }, { "WWW-Authenticate": authChallenge(RESOURCE_METADATA) });
  if (method === "initialize") return reply(200, rpc(id, { protocolVersion: modern ? "2026-07-28" : requested, serverInfo: { name: "ARIA MCP Server", version: "1.3.7" }, capabilities: { tools: { listChanged: false } }, transport: TRANSPORT }));
