...
const html = (body: string, traceId: string) => new Response(body, { status: 200, headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-store", ...traceHeaders(traceId) } });
...
Deno.serve(async req => {
  ...
  if (req.method === "POST" && u.pathname.endsWith("/register")) {
    let body: any; try { body = await req.json(); } catch { await recordTrace({ traceId, stage: "oauth.register", component: "aria-mcp-oauth-grok-v2", outcome: "error", method: "POST", path: u.pathname, statusCode: 400, errorCode: "invalid_client_metadata" }); return json(400, { error: "invalid_client_metadata" }, traceId); }
  ...
  if (req.method === "GET" && u.pathname.endsWith("/authorize")) {
    ...
    const pendingId = crypto.randomUUID(); const { error } = await db().from("aria_mcp_oauth_pending").insert({ id: pendingId, client_id: clientId, redirect_uri: redirectUri, state, code_challenge: challenge, code_challenge_method: method, trace_id: traceId, expires_at: new Date(Date.now() + PENDING_TTL_MS).toISOString() });
    if (error) return json(500, { error: "authorization_state_failed" }, traceId);
    await recordTrace({ traceId, stage: "oauth.authorize", component: "aria-mcp-oauth-grok-v2", outcome: "redirect", method: "GET", path: u.pathname, statusCode: 302, clientId, latencyMs: Date.now() - started, details: { pending_id_present: true, pkce: method } });
    const ui = new URL(`${ISSUER}/authorize/ui`); ui.searchParams.set("pending_id", pendingId); return Response.redirect(ui.toString(), 302);
  }
  if (req.method === "GET" && u.pathname.endsWith("/authorize/ui")) {
    const pendingId = u.searchParams.get("pending_id") ?? ""; const { data: pending } = await db().from("aria_mcp_oauth_pending").select("id,client_id,expires_at,trace_id").eq("id", pendingId).maybeSingle();
    const flowTrace = pending?.trace_id ?? traceId;
    if (!pending || new Date(pending.expires_at).getTime() <= Date.now()) return json(400, { error: "authorization_expired" }, flowTrace); const { data: client } = await db().from("aria_mcp_oauth_clients").select("client_name").eq("client_id", pending.client_id).maybeSingle();
    await recordTrace({ traceId: flowTrace, stage: "oauth.ui", component: "aria-mcp-oauth-grok-v2", outcome: "success", method: "GET", path: u.pathname, statusCode: 200, clientId: pending.client_id, latencyMs: Date.now() - started });
  ...
  if (req.method === "POST" && u.pathname.endsWith("/authorize/start")) {
    ...
    const { error } = await authClient().auth.signInWithOtp({ email, options: { shouldCreateUser: false } }); if (error) { await recordTrace({ traceId: flowTrace, stage: "oauth.otp", component: "aria-mcp-oauth-grok-v2", outcome: "error", method: "POST", path: u.pathname, statusCode: 400, clientId: pending.client_id, errorCode: "otp_send_failed", latencyMs: Date.now() - started }); return json(400, { error: "otp_send_failed" }, flowTrace); }
    await db().from("aria_mcp_oauth_pending").update({ email }).eq("id", pendingId); await recordTrace({ traceId: flowTrace, stage: "oauth.otp", component: "aria-mcp-oauth-grok-v2", outcome: "sent", method: "POST", path: u.pathname, statusCode: 200, clientId: pending.client_id, latencyMs: Date.now() - started });
  ...
  if (req.method === "POST" && u.pathname.endsWith("/authorize/verify")) {
    const form = await req.formData(), pendingId = String(form.get("pending_id") ?? ""), otp = String(form.get("token") ?? "").trim(); const { data: pending } = await db().from("aria_mcp_oauth_pending").select("*").eq("id", pendingId).maybeSingle(); const flowTrace = pending?.trace_id ?? traceId;
    ...
    const { data: verified, error } = await authClient().auth.verifyOtp({ email: pending.email, token: otp, type: "email" }); if (error || !verified.session || !verified.user) { await recordTrace({ traceId: flowTrace, stage: "oauth.callback", component: "aria-mcp-oauth-grok-v2", outcome: "rejected", method: "POST", path: u.pathname, statusCode: 400, clientId: pending.client_id, errorCode: "invalid_grant", latencyMs: Date.now() - started }); return json(400, { error: "invalid_grant" }, flowTrace); }
    ...
    const redirect = new URL(pending.redirect_uri); redirect.searchParams.set("code", code); redirect.searchParams.set("state", pending.state); redirect.searchParams.set("iss", ISSUER); await recordTrace({ traceId: flowTrace, stage: "oauth.callback", component: "aria-mcp-oauth-grok-v2", outcome: "redirect", method: "POST", path: u.pathname, statusCode: 302, clientId: pending.client_id, latencyMs: Date.now() - started, details: { code_issued: true } }); return Response.redirect(redirect.toString(), 302);
  }
  if (req.method === "POST" && u.pathname.endsWith("/token")) {
    ...
    const accessToken = await decryptSecret(record.encrypted_access_token); await db().from("aria_mcp_oauth_codes").update({ used_at: new Date().toISOString() }).eq("code", body.code); await recordTrace({ traceId: flowTrace, stage: "oauth.token", component: "aria-mcp-oauth-grok-v2", outcome: "issued", method: "POST", path: u.pathname, statusCode: 200, clientId: body.client_id, latencyMs: Date.now() - started, details: { token_type: "Bearer", expires_in: 3600 } });
