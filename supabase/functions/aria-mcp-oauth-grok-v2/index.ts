...
const MCP_RESOURCE = "https://aria.robvg9.workers.dev/mcp";
const JSON_HEADERS = { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" };
const HTML_HEADERS = { "content-type": "application/xhtml+xml; charset=utf-8", "cache-control": "no-store" };
...
function metadata() { return { issuer: ISSUER, resource: MCP_RESOURCE, authorization_endpoint: `${ISSUER}/authorize`, token_endpoint: `${ISSUER}/token`, registration_endpoint: `${ISSUER}/register`, response_types_supported: ["code"], grant_types_supported: ["authorization_code"], code_challenge_methods_supported: ["S256"], token_endpoint_auth_methods_supported: ["none"], scopes_supported: ["openid", "profile", "email"], client_id_metadata_document_supported: false }; }
function authorizationPage(pendingId: string) { return `<?xml version="1.0" encoding="UTF-8"?><html xmlns="http://www.w3.org/1999/xhtml" lang="en"><head><meta name="viewport" content="width=device-width,initial-scale=1"/><meta charset="utf-8"/><title>Authorize ARIA</title><style type="text/css">body{font-family:system-ui,sans-serif;max-width:520px;margin:40px auto;padding:0 16px}input,button{font:inherit;width:100%;box-sizing:border-box;padding:10px;margin-top:8px}button{cursor:pointer}</style></head><body><h1>Authorize ARIA</h1><p>ARIA will request permission to use your ChatBending context and memory tools.</p><form method="post" action="${ISSUER}/authorize/start"><input type="hidden" name="pending_id" value="${pendingId}"/><label>Email<br/><input name="email" type="email" required="required" autocomplete="email"/></label><button type="submit">Continue with email</button></form></body></html>`; }
function checkEmailPage(email: string) { const safe = email.replace(/[<>&"]/g, ""); return `<?xml version="1.0" encoding="UTF-8"?><html xmlns="http://www.w3.org/1999/xhtml" lang="en"><head><meta name="viewport" content="width=device-width,initial-scale=1"/><meta charset="utf-8"/><title>Check your email</title></head><body style="font-family:system-ui,sans-serif;max-width:520px;margin:40px auto;padding:0 16px"><h1>Check your email</h1><p>We sent a sign-in link to ${safe}.</p><p>Open the link in this email to authorize ARIA.</p></body></html>`; }
function magicPage(pendingId: string) { return `<?xml version="1.0" encoding="UTF-8"?><html xmlns="http://www.w3.org/1999/xhtml" lang="en"><head><meta name="viewport" content="width=device-width,initial-scale=1"/><meta charset="utf-8"/><title>Completing ARIA authorization</title><style type="text/css">body{font-family:system-ui,sans-serif;max-width:520px;margin:40px auto;padding:0 16px}</style></head><body><h1 id="title">Completing authorization…</h1><p id="message">Please wait while ARIA completes the secure sign-in.</p><script type="application/javascript">(async()=>{try{const hash=new URLSearchParams(location.hash.replace(/^#/,""));const accessToken=hash.get("access_token");if(!accessToken)throw new Error("No Supabase session token was returned.");const r=await fetch(${JSON.stringify(ISSUER)}+"/authorize/consume",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({pending_id:${JSON.stringify(pendingId)},access_token:accessToken})});const d=await r.json();if(!r.ok||!d.redirect_uri)throw new Error(d.error||"authorization_failed");location.replace(d.redirect_uri)}catch(e){document.getElementById("title").textContent="Authorization failed";document.getElementById("message").textContent=String(e&&e.message?e.message:e)}})();</script></body></html>`; }
...
Deno.serve(async req => {
  ...
  if (req.method === "GET" && u.pathname.endsWith("/authorize")) {
    const clientId = u.searchParams.get("client_id") ?? ""; const redirectUri = u.searchParams.get("redirect_uri") ?? ""; const responseType = u.searchParams.get("response_type") ?? ""; const state = u.searchParams.get("state") ?? ""; const challenge = u.searchParams.get("code_challenge") ?? ""; const method = u.searchParams.get("code_challenge_method") ?? "";
  ...
  if (req.method === "POST" && u.pathname.endsWith("/authorize/start")) {
    const form = await req.formData(); const pendingId = String(form.get("pending_id") ?? ""); const email = String(form.get("email") ?? "").trim().toLowerCase();
    const { data: pending } = await db().from("aria_mcp_oauth_pending").select("id,expires_at,trace_id,client_id").eq("id", pendingId).maybeSingle(); const flowTrace = pending?.trace_id ?? traceId;
  ...
  if (req.method === "GET" && u.pathname.endsWith("/authorize/magic")) {
    const pendingId = u.searchParams.get("pending_id") ?? ""; const { data: pending } = await db().from("aria_mcp_oauth_pending").select("id,expires_at,trace_id").eq("id", pendingId).maybeSingle(); const flowTrace = pending?.trace_id ?? traceId;
    if (!pending || new Date(pending.expires_at).getTime() <= Date.now()) return html(400, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><html xmlns=\"http://www.w3.org/1999/xhtml\" lang=\"en\"><head><title>Authorization expired</title></head><body><h1>Authorization expired</h1><p>Start again from Claude.</p></body></html>", flowTrace);
  ...
  if (req.method === "POST" && u.pathname.endsWith("/authorize/consume")) {
    ...
    const pendingId = String(body.pending_id ?? ""); const accessToken = String(body.access_token ?? ""); if (!pendingId || !accessToken) return json(400, { error: "invalid_request" }, traceId);
    ...
    const { data: { user }, error: userError } = await authClient().auth.getUser(accessToken); if (userError || !user) return json(400, { error: "invalid_session" }, flowTrace);
    ...
    if (insertError) return json(500, { error: "authorization_failed" }, flowTrace); await db().from("aria_mcp_oauth_pending").delete().eq("id", pendingId);
  ...
  if (req.method === "POST" && u.pathname.endsWith("/token")) {
    let body: any; const ct = req.headers.get("content-type") ?? ""; try { body = ct.includes("application/x-www-form-urlencoded") ? Object.fromEntries(new URLSearchParams(await req.text())) : await req.json(); } catch { return json(400, { error: "invalid_request" }, traceId); }
    ...
    const { data: record } = await db().from("aria_mcp_oauth_codes").select("*").eq("code", body.code).maybeSingle(); const flowTrace = record?.trace_id ?? traceId;
    if (!record || record.used_at || new Date(record.expires_at).getTime() <= Date.now() || record.client_id !== body.client_id || record.redirect_uri !== body.redirect_uri) return json(400, { error: "invalid_grant" }, flowTrace);
    if (!(await pkceMatches(body.code_verifier, record.code_challenge))) return json(400, { error: "invalid_grant" }, flowTrace);
    await db().from("aria_mcp_oauth_codes").update({ used_at: new Date().toISOString() }).eq("code", body.code);
    return json(200, { access_token: await encryptSecret(record.encrypted_access_token), token_type: "Bearer", expires_in: 3600, scope: record.scope, resource: MCP_RESOURCE }, flowTrace);
  }
  await recordTrace(db(), { traceId, stage: "oauth", method: req.method, path: u.pathname, durationMs: Date.now(), metadata: { resource: MCP_RESOURCE } });
