...
Deno.serve(async req => {
  ...
  if (req.method === "GET" && u.pathname.endsWith("/authorize/magic")) {
    ...
    const flowTrace = pending?.trace_id ?? traceId;
    if (!pending || new Date(pending.expires_at).getTime() <= Date.now()) return html(400, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><html xmlns=\"http://www.w3.org/1999/xhtml\" lang=\"en\"><head><title>Authorization expired</title></head><body><h1>Authorization expired</h1><p>Start again from Grok.</p></body></html>", flowTrace);
