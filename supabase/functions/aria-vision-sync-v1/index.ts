...
Deno.serve(async req => {
  ...
  const { error: me } = await internal.from('vision_manifest').upsert({ vision_id:visionId, source_type:'notion', source_ref:sourceRef, source_updated_at:sourceUpdatedAt, source_version:sourceVersion, content_hash:contentHash, snapshot_metadata:{ compiler_status:'live', sync:'notion_to_vision_manifest', received_at:new Date().toISOString(), objective_count:objectives.length }, active:true, updated_at:new Date().toISOString() }, { onConflict:'vision_id' });
