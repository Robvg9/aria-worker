...
async function learn(){const cut=new Date(Date.now()-6*60*60*1000).toISOString();const {data,error}=await sb.schema("aria_internal").from("mission_state").select("mission_id,goal,status,metadata,last_stderr,last_stdout,updated_at,created_at").in("status",["succeeded","blocked","failed","timeout","cancelled"]).gt("updated_at",cut);if(error)throw new Error(error.message);let created=0;for(const m of data||[]){const goalId=m.metadata?.goal_id??null;const {data:e}=await sb.schema("aria_internal").from("autonomy_learnings").select("lesson_id").eq("mission_id",m.mission_id).limit(1);if(!e?.length){const {error:ie}=await sb.schema("aria_internal").from("autonomy_learnings").insert({mission_id:m.mission_id,goal_id:goalId,category:m.status==="succeeded"?"verified_success":"operational_failure",summary:`Observed ${m.status}: ${(m.goal||"").slice(0,220)}`,evidence:{status:m.status,stderr:m.last_stderr||null,stdout_sample:(m.last_stdout||"").slice(0,800)},confidence:m.status==="succeeded"?0.9:0.75,reusable:true});if(!ie)created++}if(goalId){const terminalStatus=m.status==="succeeded"?"completed":"blocked";const {error:ge}=await sb.schema("aria_internal").from("autonomy_goals").update({status:terminalStatus,updated_at:new Date().toISOString()}).eq("goal_id",goalId).in("status",["running","queued","paused"]);if(ge)throw new Error(ge.message)}}return {scanned:data?.length||0,created}}
async function dynamicGoalPool(){
  const now=new Date().toISOString();
  const failureCut=new Date(Date.now()-72*60*60*1000).toISOString();
  const [goalsRes, failuresRes, gapsRes, learningsRes] = await Promise.all([
    ...
    sb.schema("aria_internal").from("mission_state").select("mission_id,goal,status,last_stderr,last_stdout,updated_at,created_at,metadata").in("status",["failed","blocked","timeout"]).gt("updated_at",failureCut).order("updated_at",{ascending:false}).limit(12),
...
async function makeMission(g:any){
  ...
  const {data:claimed,error:ce}=await sb.schema("aria_internal").from("autonomy_goals").update({status:"running",attempts:(g.attempts||0)+1,updated_at:claimedAt}).eq("goal_id",g.goal_id).eq("status","queued").select("*").maybeSingle();
  if(ce)throw new Error(ce.message);
  if(!claimed)return null;
  const id=`mission_${crypto.randomUUID()}`,sd=g.goal_id==="autonomy-selfdevelopment-audit";
  try{
    const {data,error}=await sb.rpc("aria_mission_create",{p_mission:{mission_id:id,status:"queued",goal:g.goal,current_step:0,completed_steps:0,checkpoint:{dynamic_goal_selection:{goal_id:g.goal_id,score:g.dynamic_score,source_type:g.source_type,source_ref:g.source_ref}},metadata:{source:"autonomy_supervisor_v10_dynamic_goal_engine_v1",goal_id:g.goal_id,policy:"safe-progress-v6",dynamic_goal:true,dynamic_score:g.dynamic_score??null,goal_source:g.source_type??null,self_development:sd,self_development_mode:sd?"audit_only":null,human_gate_required:["HIGH_RISK_WRITE","destructive","production_merge"]}}});
    if(error)throw new Error(error.message);
    const {error:ue}=await sb.schema("aria_internal").from("autonomy_goals").update({last_mission_id:id,updated_at:new Date().toISOString()}).eq("goal_id",g.goal_id).eq("status","running");
    if(ue)throw new Error(ue.message);
    return data;
  }catch(error){
    await sb.schema("aria_internal").from("autonomy_goals").update({status:"queued",attempts:Math.max(0,(claimed.attempts||1)-1),updated_at:new Date().toISOString()}).eq("goal_id",g.goal_id).eq("last_mission_id",null);
    throw error;
  }
}
