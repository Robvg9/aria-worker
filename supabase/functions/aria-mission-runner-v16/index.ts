...
async function cloudflareRead(operation:string){const r=await fetch(`${WORKER}/admin/cloudflare?operation=${encodeURIComponent(operation)}`,{headers:{authorization:`Bearer ${SECRET}`}});const j=await r.json().catch(()=>null);if(!r.ok||j?.error)throw new Error(`cloudflare_admin_${r.status}`);return {...j,executor_type:"connector",connector_id:"cloudflare",operation}}
...
async function executeStep(missionId:string,step:any,attempt:number){const type=String(step.executor_type||step.target?.type||'connector');if(type==='device')return deviceExecute(missionId,step,attempt);if(type==='agent')throw new Error('agent_executor_requires_registered_runtime');if(type!=='connector')throw new Error(`unsupported_executor:${type}`);const cid=String(step.target?.connector_id||step.connector_id||'');const op=String(step.operation||'');if(cid==='github')return githubRead(String(step.input?.path||step.path||'README.md'),op||'file_read');if(cid==='cloudflare')return cloudflareRead(op||'worker_read');if(cid==='supabase')return supabaseRead(missionId,op||'mission_read');throw new Error(`unsupported_connector:${cid}`)}
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    for(const step of plan){const sid=String(step.id);if(completed.has(sid))continue;const deps=Array.isArray(step.depends_on)?step.depends_on.map(String):[];if(!deps.every((d:string)=>completed.has(d))){await update(id,{status:'blocked',next_action:'recovery: dependency graph waiting',lease_owner:null,lease_until:null});return out({ok:false,status:'blocked',reason:'dependencies_unsatisfied',mission_id:id})}
      ...
      while(attempt<maxAttempts&&!success){attempt+=1;attempts[sid]=attempt;await emit(id,'step_started',{step_id:sid,operation:step.operation,executor_type:step.executor_type||step.target?.type||'connector',attempt});let result:any;try{result=await executeStep(id,step,attempt)}catch(e){result={status:'failed',stderr:e instanceof Error?e.message:String(e),executor_type:step.executor_type||step.target?.type||'connector',operation:step.operation,attempt}}
