...
Deno.serve(async (request) => {
  ...
  if (error || rows !== true) return out({ error: "invalid_or_consumed_nonce" }, 401);
  ...
  try {
    ...
    return out({ ok: false, provider: "google", route: "direct", status: "failed", reason: "transport_error", latency_ms: Date.now() - started }, 502);
  }
