...
async function executionJobDbCall(kind:string,args:any){
  const dbUrl=Deno.env.get('SUPABASE_DB_URL')||'';
  if(!dbUrl)throw new Error('execution_job_db_url_unavailable');
  let sql:any=null;
  try{
    ...
    if(kind==='start'){
      const rows=await sql`select public.start_execution_job_gateway(${String(args.jobId)},${String(args.deviceId)}) as job`;
      return rows?.[0]?.job??null;
    }
    if(kind==='complete'){
    ...
    throw new Error('execution_job_db_call_unsupported');
  }finally{
...
if(req.method==='GET'&&ideaMatch){try{return json({ok:true,proposal:await meditationIdeaProposalById(d.device_id,decodeURIComponent(ideaMatch[1]))})}catch(e){return json({ok:false,error:queueError(e)},404)}}const sr=p.match(/^\/v1\/jobs\/([^/]+)\/resolve-secret$/);if(req.method==='POST'&&sr){try{const secretRef=typeof b.secret_ref==='string'?b.secret_ref:'';return json(await resolveRwhtSecret(decodeURIComponent(sr[1]),d,secretRef))}catch(e){return json({ok:false,error:queueError(e)},409)}}if(req.method==='POST'&&p==='/v1/jobs/claim'){try{const data=await executionJobDbCall('claim',{deviceId:d.device_id});return json({job:data??null,transport:'postgres'})}catch(e){const fb=await supabase.rpc('claim_execution_job_gateway',{p_device_id:d.device_id});if(!fb.error)return json({job:fb.data??null,transport:'postgrest-fallback'});return json({error:'claim_failed',detail:queueError(e)},500)}}const s=p.match(/^\/v1\/jobs\/([^/]+)\/start$/);if(req.method==='POST'&&s){const jobId=decodeURIComponent(s[1]);try{const data=await executionJobDbCall('start',{jobId,deviceId:d.device_id});if(!data)return json({error:'job_not_owned_or_not_claimed'},409);return json({ok:true,job:data,transport:'postgres'})}catch(e){const fb=await supabase.rpc('start_execution_job_gateway',{p_job_id:jobId,p_device_id:d.device_id});if(!fb.error&&fb.data)return json({ok:true,job:fb.data,transport:'postgrest-fallback'});return json({error:'start_failed',detail:queueError(e)},500)}}const z=p.match(/^\/v1\/jobs\/([^/]+)\/result$/);if(req.method==='POST'&&z){const jobId=decodeURIComponent(z[1]);if(!b.result||typeof b.result!=='object')return json({error:'result_required'},400);const r=b.result as Record<string,unknown>;const status=['succeeded','failed','timeout','cancelled'].includes(String(r.status))?String(r.status):'failed';const exitCode=Number.isInteger(r.exit_code)?Number(r.exit_code):null;const baseMetadata=r.metadata&&typeof r.metadata==='object'?r.metadata:{};const autonomousInner=r.result&&typeof r.result==='object'?(r.result as Record<string,unknown>):null;const autonomousSummary=baseMetadata.autonomous_test===true&&autonomousInner?{status:String(autonomousInner.status||status),reason:typeof autonomousInner.reason==='string'?autonomousInner.reason:null,steps:Number.isFinite(Number(autonomousInner.steps))?Number(autonomousInner.steps):null,evidence:Array.isArray(autonomousInner.evidence)?autonomousInner.evidence.slice(0,64):[],trace:Array.isArray(autonomousInner.trace)?autonomousInner.trace.slice(0,64).map((x:any)=>({step:x?.step??null,decision:x?.decision??null,reason:typeof x?.reason==='string'?x.reason.slice(0,700):null,action:x?.action??null,execution:x?.execution??null,before_evidence_hash:x?.before_evidence_hash??null})):[]} : null;const clean={status,exit_code:exitCode,stdout:typeof r.stdout==='string'?r.stdout:'',stderr:typeof r.stderr==='string'?r.stderr:'',duration_ms:Number.isFinite(Number(r.duration_ms))?Number(r.duration_ms):null,metadata:autonomousSummary?{...baseMetadata,autonomous_summary:autonomousSummary}:baseMetadata};try{const data=await executionJobDbCall('complete',{jobId,deviceId:d.device_id,status,exitCode,stdout:clean.stdout,stderr:clean.stderr,result:clean});if(!data)return json({error:'job_not_owned_or_already_finished'},409);return json({ok:true,job:{job_id:jobId,status},transport:'postgres'})}catch(e){const fb=await supabase.rpc('complete_execution_job_gateway',{p_job_id:jobId,p_device_id:d.device_id,p_status:status,p_exit_code:exitCode,p_stdout:clean.stdout,p_stderr:clean.stderr,p_result:clean});if(!fb.error&&fb.data)return json({ok:true,job:{job_id:jobId,status},transport:'postgrest-fallback'});return json({error:'result_failed',detail:queueError(e)},500)}}return json({error:'not_found'},404)})