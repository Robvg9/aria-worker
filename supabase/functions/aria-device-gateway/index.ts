...
async function androidAutonomousDecision(b:any,d:any){
  ...
  // Only LIVE-verified routes. Do not list models absent from model_registry / capability_matrix.
...
async function recordComputerUseProgress(jobId:string,deviceId:string,event:any){
  ...
  if(!COMPUTER_USE_PROGRESS_TYPES.has(type)) throw new Error('computer_use_progress_type_not_allowed');
  const payload=event?.payload&&typeof event.payload==='object'?event.payload:{};
  const {data:job,error:je}=await supabase.schema('aria_internal')
    .from('execution_jobs')
    .select('job_id,mission_id,device_id,status,operation')
    .eq('job_id',jobId)
    .eq('device_id',deviceId)
    .maybeSingle();
  if(je) throw new Error(je.message);
  if(!job) throw new Error('job_not_owned_or_missing');
  if(String(job.operation)!=='computer.use.autonomous') throw new Error('job_operation_not_autonomous');
  if(!job.mission_id) throw new Error('job_mission_id_missing');
  const rawStep=event?.step_index;
  const stepIndex=Number.isInteger(Number(rawStep))&&Number(rawStep)>0?Number(rawStep):null;
  const safePayload={
    ...payload,
    job_id:jobId,
    device_id:deviceId,
    operation:'computer.use.autonomous',
    progress_version:'computer-use-live-v1',
  };
  const {data:row,error}=await supabase.schema('aria_internal').from('mission_events')
    .insert({
  ...
  await supabase.schema('aria_internal').from('execution_job_events').insert({
    job_id:jobId,
    device_id:deviceId,
...
async function mission5ModelProbe(b:any,d:any){
  ...
  // Only routes that exist in model_registry with status=available + verified capability.
...
async function meditationTick(b:any,d:any){const sessionId=String(b?.session_id||'').trim();const deviceId=String(d.device_id||'').trim();const recovered=await recoverStale();const learning=await learnRecent();const goalSync=await syncGoalTerminalStates();const autonomyOnly=b?.autonomy_only===true;
...
// Goal deduplication is global for inflight mission_state rows; do not narrow by UI session.
...
// Duplicate recovery must honor the same goal-level uniqueness boundary.
...
if(duplicateInflight || String(me.message||'').includes('mission_state_one_inflight_goal_idx')){
  return {ok:true,status:'raced',mission_created:null,active_mission_id:null,goal:selected.goal,goal_id:selected.goal_id,dynamic_score:selected.dynamic_score,goal_source:selected.source_type,race_recovered:true,recovered,learning,goalSync};
}
...
const a=await auth(req,b.device_id);if(a.error)return a.error;const d=a.device;if(req.method==='POST'&&p==='/v1/devices/heartbeat'){const agentType=String(b.agent_type||d.agent_type);const caps=new Set(Array.isArray(b.capabilities)?b.capabilities.map(String):Array.isArray(d.capabilities)?d.capabilities.map(String):[]);if(agentType==='android-termux'){caps.delete('computer.use.android');const h=b.android_ui_health&&typeof b.android_ui_health==='object'?b.android_ui_health:null;if(h?.ok===true&&h.protocol==='aria-android-ui-agent-ipc-v2')caps.add('computer.use.android');}const {error}=await supabase.rpc('heartbeat_device_gateway',{p_device_id:d.device_id,p_capabilities:Array.from(caps),p_agent_type:agentType});if(error)return json({error:'heartbeat_failed'},500);return json({ok:true,device_id:d.device_id,capabilities:Array.from(caps)})}if(req.method==='POST'&&p==='/v1/android/autonomous/decide'){try{return await androidAutonomousDecision(b,d)}catch(e){return json({ok:false,error:queueError(e)},502)}}if(req.method==='POST'&&p==='/v1/router/decide'){try{return json(await intelligentRouterDecision({...b,device_id:d.device_id}))}catch(e){return json({status:'failed',error:queueError(e)},500)}}
...
if(req.method==='GET'&&ideaMatch){try{return json({ok:true,proposal:await meditationIdeaProposalById(d.device_id,decodeURIComponent(ideaMatch[1]))})}catch(e){return json({ok:false,error:queueError(e)},404)}}const sr=p.match(/^\/v1\/jobs\/([^/]+)\/resolve-secret$/);if(req.method==='POST'&&sr){try{const secretRef=typeof b.secret_ref==='string'?b.secret_ref:'';return json(await resolveRwhtSecret(decodeURIComponent(sr[1]),d,secretRef))}catch(e){return json({ok:false,error:queueError(e)},409)}}if(req.method==='POST'&&p==='/v1/jobs/claim'){try{const data=await executionJobGatewayCall('claim',{deviceId:d.device_id});return json({job:data??null,transport:'supabase-rpc'})}catch(e){return json({error:'claim_failed',detail:queueError(e)},503)}}const s=p.match(/^\/v1\/jobs\/([^/]+)\/start$/);if(req.method==='POST'&&s){const jobId=decodeURIComponent(s[1]);try{const data=await executionJobGatewayCall('start',{jobId,deviceId:d.device_id});if(!data)return json({error:'job_not_owned_or_not_claimed'},409);return json({ok:true,job:data,transport:'supabase-rpc'})}catch(e){return json({error:'start_failed',detail:queueError(e)},503)}}const gp=p.match(/^\/v1\/jobs\/([^/]+)\/progress$/);if(req.method==='POST'&&gp){const jobId=decodeURIComponent(gp[1]);try{const row=await recordComputerUseProgress(jobId,d.device_id,b?.event||b);return json({ok:true,event:row})}catch(e){return json({ok:false,error:queueError(e)},409)}}const z=p.match(/^\/v1\/jobs\/([^/]+)\/result$/);if(req.method==='POST'&&z){const jobId=decodeURIComponent(z[1]);if(!b.result||typeof b.result!=='object')return json({error:'result_required'},400);const r=b.result as Record<string,unknown>;const status=['succeeded','failed','timeout','cancelled'].includes(String(r.status))?String(r.status):'failed';const exitCode=Number.isInteger(r.exit_code)?Number(r.exit_code):null;const baseMetadata=r.metadata&&typeof r.metadata==='object'?r.metadata:{};const autonomousInner=r.result&&typeof r.result==='object'?(r.result as Record<string,unknown>):null;const autonomousSummary=baseMetadata.autonomous_test===true&&autonomousInner?{status:String(autonomousInner.status||status),reason:typeof autonomousInner.reason==='string'?autonomousInner.reason:null,steps:Number.isFinite(Number(autonomousInner.steps))?Number(autonomousInner.steps):null,evidence:Array.isArray(autonomousInner.evidence)?autonomousInner.evidence.slice(0,64):[],trace:Array.isArray(autonomousInner.trace)?autonomousInner.trace.slice(0,64).map((x:any)=>({step:x?.step??null,decision:x?.decision??null,reason:typeof x?.reason==='string'?x.reason.slice(0,700):null,action:x?.action??null,execution:x?.execution??null,before_evidence_hash:x?.before_evidence_hash??null})):[]} : null;const clean={status,exit_code:exitCode,stdout:typeof r.stdout==='string'?r.stdout:'',stderr:typeof r.stderr==='string'?r.stderr:'',duration_ms:Number.isFinite(Number(r.duration_ms))?Number(r.duration_ms):null,metadata:autonomousSummary?{...baseMetadata,autonomous_summary:autonomousSummary}:baseMetadata};try{const data=await executionJobGatewayCall('complete',{jobId,deviceId:d.device_id,status,exitCode,stdout:clean.stdout,stderr:clean.stderr,result:clean});if(!data)return json({error:'job_not_owned_or_already_finished'},409);return json({ok:true,job:{job_id:jobId,status},transport:'supabase-rpc'})}catch(e){return json({error:'result_failed',detail:queueError(e)},503)}}return json({error:'not_found'},404)})