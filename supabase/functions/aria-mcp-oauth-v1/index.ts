...
async function deriveKey() {
  const raw = await sha256(SERVICE_ROLE_KEY);
...
async function decryptSecret(value: string) {
  const [iv64, ct64] = value.split(".");
  const key = await deriveKey();
  const plaintext = await crypto.subtle.decrypt({ name: "AES-GCM", iv: fromB64url(iv64) }, key, fromB64url(ct64));
...
Deno.serve(async req => {
  ...
  if (req.method === "POST" && u.pathname.endsWith("/register")) {
    ...
    for (const uri of redirectUris) {
      if (!/^https:\/\//.test(uri) && !/^http:\/\/127\.0\.0\.1(?::\d+)?\//.test(uri) && !/^http:\/\/localhost(?::\d+)?\//.test(uri)) return json(400, { error: "invalid_redirect_uri" });
    }
  ...
  if (req.method === "POST" && u.pathname.endsWith("/authorize/start")) {
    ...
    if (!pendingId || !/^\S+@\S+\.\S+$/.test(email)) return json(400, { error: "invalid_request" });
    const { data: pending } = await db().from("aria_mcp_oauth_pending").select("id,expires_at").eq("id", pendingId).maybeSingle();
  ...
  if (req.method === "POST" && u.pathname.endsWith("/authorize/verify")) {
    const form = await req.formData();
    const pendingId = String(form.get("pending_id") ?? "");
    const otp = String(form.get("token") ?? "").trim();
    const { data: pending } = await db().from("aria_mcp_oauth_pending").select("*").eq("id", pendingId).maybeSingle();
    if (!pending || new Date(pending.expires_at).getTime() <= Date.now() || !pending.email || !/^\d{6,8}$/.test(otp)) return json(400, { error: "invalid_authorization" });
    const { data: verified, error } = await authClient().auth.verifyOtp({ email: pending.email, token: otp, type: "email" });
    if (error || !verified.session || !verified.user) return json(400, { error: "invalid_grant" });
  ...
  if (req.method === "POST" && u.pathname.endsWith("/token")) {
    ...
    try { body = ct.includes("application/x-www-form-urlencoded") ? Object.fromEntries(new URLSearchParams(await req.text())) : await req.json(); } catch { return json(400, { error: "invalid_request" }); }
    if (body.grant_type !== "authorization_code" || typeof body.code !== "string" || typeof body.code_verifier !== "string" || typeof body.client_id !== "string" || typeof body.redirect_uri !== "string") return json(400, { error: "invalid_request" });
    const { data: record } = await db().from("aria_mcp_oauth_codes").select("*").eq("code", body.code).maybeSingle();
    if (!record || record.used_at || new Date(record.expires_at).getTime() <= Date.now()) return json(400, { error: "invalid_grant" });
