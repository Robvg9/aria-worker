...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET" && u.pathname.endsWith("/authorize")) {
    ...
    const client = await clientFromRequest(clientId, redirectUri);
    if (!client) return json(400, { error: "invalid_client" }, traceId);
  ...
  if (req.method === "POST" && u.pathname.endsWith("/authorize/start")) {
    ...
    return html(200, checkPage(email), flowTrace);
  }
  if (req.method === "GET" && u.pathname.endsWith("/authorize/magic")) {
    const pendingId = u.searchParams.get("pending_id") ?? "";
    const { data: pending } = await db().from("aria_mcp_oauth_pending").select("id,expires_at,trace_id").eq("id", pendingId).maybeSingle();
    if (!pending || new Date(pending.expires_at).getTime() <= Date.now()) return html(400, "<h1>Authorization expired</h1>", traceId);
    return html(200, magicPage(pendingId), pending.trace_id ?? traceId);
  }
  if (req.method === "POST" && u.pathname.endsWith("/authorize/consume")) {
    ...
    if (userError || !userData.user) return json(400, { error: "invalid_session" }, flowTrace);
    const code = `aria_code_${crypto.randomUUID()}`;
    const { data: secretBytes } = await db().rpc("aria_store_mcp_oauth_secret", { p_code: code, p_access_token: accessToken, p_client_id: pending.client_id, p_redirect_uri: pending.redirect_uri, p_code_challenge: pending.code_challenge, p_user_id: userData.user.id, p_scope: SCOPES.join(" "), p_trace_id: flowTrace, p_expires_at: new Date(Date.now() + codeTtl).toISOString() });
    if (secretBytes === null && !userData.user.id) return json(500, { error: "authorization_failed" }, flowTrace);
    const redirect = new URL(pending.redirect_uri); redirect.searchParams.set("code", code); redirect.searchParams.set("state", pending.state); redirect.searchParams.set("iss", ISSUER); await db().from("aria_mcp_oauth_pending").delete().eq("id", pendingId);
    return json(200, { redirect_uri: redirect.toString() }, flowTrace);
  }
  if (req.method === "POST" && u.pathname.endsWith("/token")) {
    const ct = req.headers.get("content-type") ?? "";
    const body = (ct.includes("application/x-www-form-urlencoded") ? Object.fromEntries(new URLSearchParams(await req.text())) : await req.json().catch(() => null)) as Record<string, unknown> | null;
    ...
    const { data: record } = await db().rpc("aria_consume_mcp_oauth_code", { p_code: code });
    const row = Array.isArray(record) ? record[0] : record as Record<string, unknown> | null;
    if (!row || row.client_id !== clientId || row.redirect_uri !== redirectUri || row.used_at || new Date(String(row.expires_at)).getTime() <= Date.now()) return json(400, { error: "invalid_grant" }, traceId);
    if (!(await pkce(verifier, String(row.code_challenge)))) return json(400, { error: "invalid_grant" }, traceId);
    const token = String(row.access_token ?? "");
    if (!token) return json(500, { error: "token_unavailable" }, traceId);
    return json(200, { access_token: token, token_type: "Bearer", scope: String(row.scope ?? SCOPES.join(" ")), expires_in: 3600 }, traceId, { "access-control-allow-origin": "*" });
  }
