...
function serviceClient() {
  ...
  return createClient(SUPABASE_URL, SERVICE_ROLE_KEY, { auth: { persistSession: false, autoRefreshToken: false, autoRefreshSession: false } });
}
...
async function callDirectAria(payload: Record<string, unknown>) {
  if (!ARIA_RUNTIME_SHARED_SECRET) return json({ error: "runtime_secret_not_configured" }, 503);
  ...
  let body: any = null;
  try { body = payloadText ? JSON.parse(payloadText) : null; } catch { body = null; }
...
function toConversationResponse(body: any, fallbackText: string) {
  ...
  const visualState = status === "queued" ? "planning" : status === "succeeded" ? "success" : status === "failed" ? "error" : status === "paused" ? "waiting" : "executing";
  return {
    ok: body?.ok !== false,
    conversationId: body?.conversation_id ?? body?.conversationId ?? crypto.randomUUID(),
...
Deno.serve(async (request) => {
  ...
  try {
    if (request.method === "GET" && pathname.endsWith("/session")) {
      return json({ ok: true, service: "aria-app-api-v1", user: { id: user.id, email: user.email ?? null } });
    }

    if (request.method === "GET" && pathname.endsWith("/system")) {
      const response = await fetch(DIRECT_ARIA, { method: "GET" });
      const body = await response.json().catch(() => null);
      return json({ ok: response.ok, service: "aria-app-api-v1", user_id: user.id, aria: body });
    }
    ...
    if (request.method === "POST" && pathname.endsWith("/missions")) {
      ...
      if (!direct.response.ok) return json({ error: direct.body?.error ?? "aria_direct_failed" }, direct.response.status);
      return json(toConversationResponse(direct.body, "Misión recibida."));
    }
