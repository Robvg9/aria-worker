...
function createFileStateStore({ root_dir } = {}) {
  ...
  async function load() {
    ...
    try {
      ...
      if (parsed?.last_checkpoint) await save(state);
      return state;
    } catch {
  ...
  async function acquireLock() {
    await ensureRoot();
    try {
      const h = await fsp.open(lockPath, 'wx');
      await h.writeFile(JSON.stringify({ pid: process.pid, acquired_at: new Date().toISOString() }), 'utf8');
      await h.close();
      return true;
    } catch (e) {
      ...
      try {
        ...
        if (!Number.isInteger(oldPid) || oldPid <= 0) {
          ...
          const h = await fsp.open(lockPath, 'wx');
          await h.writeFile(JSON.stringify({ pid: process.pid, acquired_at: new Date().toISOString(), reclaimed: true, reason: 'invalid_pid' }), 'utf8');
          await h.close();
          return true;
        }
        if (oldPid === process.pid) {
          await fsp.rm(lockPath, { force: true });
          const h = await fsp.open(lockPath, 'wx');
          await h.writeFile(JSON.stringify({ pid: process.pid, acquired_at: new Date().toISOString(), reclaimed: true, reason: 'same_pid' }), 'utf8');
          await h.close();
          return true;
        }
        try {
          ...
          await fsp.rm(lockPath, { force: true });
          const h = await fsp.open(lockPath, 'wx');
          await h.writeFile(JSON.stringify({ pid: process.pid, acquired_at: new Date().toISOString(), reclaimed: true }), 'utf8');
          await h.close();
          return true;
        }
      } catch {
...
function snapshotWithoutCheckpoint(value) {
  const snapshot = { ...value };
  snapshot.last_checkpoint = null;
  return snapshot;
}
...
function createMeditationController({
  ...
  async function handleCommand(command) {
    ...
    if (c === 'START' || c === 'AVANZA' || c === 'CONTINUA') {
      ...
      if (!locked) {
        locked = await stateStore.acquireLock();
        if (!locked) return { status: 'locked', reason: 'another_meditation_controller_is_active' };
      }
      setMode('active');
      state = { ...state, session_id: state.session_id || `med-${now().getTime()}-${process.pid}`, started_at: state.started_at || now().toISOString(), paused_at: null, stopped_at: null, last_checkpoint: null };
      await persist();
      try { await ensureNotepad(); } catch (error) { await write(`NOTEPAD_START_ERROR ${String(error?.message || error)}`); }
      await write(`MODE ACTIVE command=${c}`);
      await onModeChange(status());
      if (!timer) timer = setInterval(() => { void tick('heartbeat').catch(error => { void write(`TICK_ASYNC_ERROR ${String(error?.message || error)}`); }); }, heartbeat_ms);
      return { status: 'resumed', command: c };
    }
    if (c === 'PAUSA') {
      setMode('paused');
      state = { ...state, paused_at: now().toISOString() };
      await persist();
      await write('MODE PAUSED command=PAUSA');
      await onModeChange(status());
      return { status: 'paused', command: c };
    }
    if (c === 'CERRAR' || c === 'DETENER') {
      setMode('stopped');
      state = { ...state, stopped_at: now().toISOString() };
      await persist();
      const cp = await checkpoint({ type: 'stop', reason: c, state: snapshotWithoutCheckpoint(status()), session_id: state.session_id });
      state = { ...state, last_checkpoint: cp };
      await persist();
      await write(`MODE STOPPED command=${c}`);
      await onModeChange(status());
      return { status: 'stopped', command: c, checkpoint: cp };
    }
    await write(`STATUS ${JSON.stringify(status())}`);
    return { status: 'reported', command: c };
  }

  async function consumeCommands() {
    const r = await commandSource.read(state.command_offset || 0);
    let next = state.command_offset || 0;
    for (const e of r?.entries || []) {
      next = Math.max(next, Number(e.next_offset || next));
      const result = await handleCommand(e.command);
      if (result.status === 'stopped') break;
    }
    if (next !== state.command_offset) {
      state = { ...state, command_offset: next };
      await persist();
    }
    return r?.entries || [];
  }
  ...
  async function tick(reason = 'scheduled') {
    if (busy) return { status: 'busy', reason };
    busy = true;
    state = { ...state, tick_count: state.tick_count + 1, last_tick_at: now().toISOString() };
    await persist();
    try {
      const entries = await consumeCommands();
      if (state.mode !== 'active') return { status: 'inactive', mode: state.mode, commands: entries.length };
      if (!(await isUserIdle()) && min_idle_seconds > 0) {
        await write(`USER_PRESENT reason=${reason} execution_deferred=true`);
      ...
      if (response?.mission_created) {
        ...
        await write(`MISSION CREATED id=${response.mission_created} goal=${JSON.stringify(response.goal || '')}`);
      } else if (response?.active_mission_id) {
        state = { ...state, active_mission_id: response.active_mission_id, active_goal: response.goal || null };
      ...
      const checkpointState = snapshotWithoutCheckpoint(status());
      const cp = await checkpoint({ type: 'tick', reason, state: checkpointState, response });
      state = { ...state, last_checkpoint: cp };
      await persist();
      return { status: 'completed', tick: state.tick_count, response, checkpoint: cp };
    } catch (error) {
      state = { ...state, last_error: String(error?.message || error), last_result: { status: 'failed', error: String(error?.message || error) } };
      await persist();
      await write(`ERROR ${state.last_error}`);
      return { status: 'failed', tick: state.tick_count, error: state.last_error };
    } finally {
      busy = false;
    }
  }
  ...
  async function start() {
    ...
    try {
      ...
      state = {
        ...state,
        mode: 'active',
        session_id: `med-${now().getTime()}-${process.pid}`,
      ...
      await persist();
      try { await ensureNotepad(); } catch (error) { await write(`NOTEPAD_START_ERROR ${String(error?.message || error)}`); }
      await write(`START session=${state.session_id} heartbeat_ms=${heartbeat_ms}`);
      await onModeChange(status());
      if (!timer) timer = setInterval(() => { void tick('heartbeat').catch(error => { void write(`TICK_ASYNC_ERROR ${String(error?.message || error)}`); }); }, heartbeat_ms);
      void tick('startup').catch(error => { void write(`STARTUP_TICK_ASYNC_ERROR ${String(error?.message || error)}`); });
      return { status: 'started', state: status() };
    } catch (error) {
      const message = String(error?.message || error);
      state = { ...state, last_error: message, last_result: { status: 'failed', error: message } };
      try { await persist(); } catch {}
      try { await write(`START_ERROR ${message}`); } catch {}
      if (acquiredHere) {
        locked = false;
        try { await stateStore.releaseLock(); } catch {}
      }
      return { status: 'failed', error: message, state: status() };
    }
  }
  ...
  async function stop() {
    const result = await handleCommand('CERRAR');
