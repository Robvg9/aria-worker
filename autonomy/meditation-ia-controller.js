...
function createFileStateStore({ root_dir } = {}) {
  ...
  async function load() {
    await ensureRoot();
    try {
      return { ...defaultState(), ...JSON.parse(await fsp.readFile(statePath, 'utf8')) };
    } catch {
  ...
  async function acquireLock() {
    ...
    try {
      ...
      if (ownerPid && oldPid !== ownerPid) {
        return reclaimLock('watchdog_owner_changed', {
          previous_pid: oldPid,
          current_agent_pid: ownerPid
        });
      }
...
function createMeditationController({
  ...
  async function handleCommand(command) {
    ...
    if (c === 'START' || c === 'AVANZA' || c === 'CONTINUA') {
      ...
      setMode('active');
      state = { ...state, session_id: state.session_id || `med-${now().getTime()}-${process.pid}`, started_at: state.started_at || now().toISOString(), paused_at: null, stopped_at: null };
      await persist();
      await ensureNotepad();
    ...
    if (c === 'CERRAR' || c === 'DETENER') {
      ...
      await persist();
      const cp = await checkpoint({ type: 'stop', reason: c, state: status(), session_id: state.session_id });
  ...
  async function consumeCommands() {
    const result = await commandSource.read(state.command_offset || 0);
    let next = state.command_offset || 0;
    for (const entry of result?.entries || []) {
      next = Math.max(next, Number(entry.next_offset || next));
      const handled = await handleCommand(entry.command);
      if (handled.status === 'stopped') break;
    }
  ...
  async function tick(reason = 'scheduled') {
    ...
    try {
      ...
      await ensureNotepad();
      const response = await requestTick({ state: status(), reason, tick: state.tick_count });
      ...
      if (failed) {
        ...
        const previous = state.failure_ledger[key] || { consecutive: 0, total: 0 };
        ...
        state = { ...state, failure_ledger: { ...state.failure_ledger, [key]: failure }, last_error: failure.last_error };
        await persist();
        await write(`FAILURE consecutive=${failure.consecutive} total=${failure.total}`);
        if (failure.consecutive >= max_consecutive_failures) {
          await write('FAILURE_THRESHOLD reached=3 action=alternate_strategy_requested');
          await onMissionEvent({ type: 'fallback_requested', consecutive_failures: failure.consecutive, mission: response, reason: 'same_goal_failed_3_times' });
        }
      }
      const cp = await checkpoint({ type: 'tick', reason, state: status(), response });
      state = { ...state, last_checkpoint: cp };
      await persist();
  ...
  async function start() {
    ...
    setMode('active');
    state = { ...state, session_id: `med-${now().getTime()}-${process.pid}`, started_at: now().toISOString(), stopped_at: null, paused_at: null };
    await persist();
    await ensureNotepad();
