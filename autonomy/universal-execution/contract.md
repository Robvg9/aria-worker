...
Version: `aria-universal-execution-v1.1.0`
...
- `model`: delegates text generation to the canonical model execution runtime. UO-1 registers `text_generation`; target requires `provider_id`, `account_id`, and `model_id`.
...
7. Executor selection never chooses a provider, account, model, credential or fallback route; those remain downstream routing/execution responsibilities.
...
- all four executor types are registered with explicit operation/target contracts;
