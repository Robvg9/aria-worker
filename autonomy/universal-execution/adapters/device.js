...
function createDeviceAdapter({ deviceDispatcher } = {}) {
  ...
  return Object.freeze({
    adapter_id: 'device-runtime-v1.2',
    ...
    async execute({ missionId, step, attempt = 1, policy, request = {}, selection } = {}) {
      ...
      if (!available) {
        return { status: 'blocked', executor_type: 'device', attempt, reason: 'device_dispatcher_unavailable' };
      }
      try {
        const result = await deviceDispatcher.execute({ missionId, step, attempt, policy, request, selection });
        return { ...result, executor_type: 'device', attempt };
      } catch (_error) {
        return { status: 'failed', executor_type: 'device', attempt, error: { code: 'adapter_error', message: 'device adapter execution failed' } };
      }
    }
  });
}
