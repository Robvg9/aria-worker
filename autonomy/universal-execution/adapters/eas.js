...
const OPERATIONS = Object.freeze([
  'eas.connection_status',
  'eas.workflow_definitions',
  'eas.workflow_list',
  'eas.workflow_info',
  'eas.workflow_dispatch',
  'eas.build_list',
  'eas.build_info',
  'eas.build_logs'
]);
  ...
  return Object.freeze({
    ...
    async execute({ missionId, step, attempt = 1, policy } = {}) {
      ...
      if (!OPERATIONS.includes(operation)) {
        return { status: 'failed', executor_type: 'eas', operation, error: { code: 'operation_not_supported', message: 'EAS operation not supported' } };
      }
      const projectId = step.target?.project_id;
      if (typeof projectId !== 'string' || !projectId.trim()) {
        return { status: 'blocked', executor_type: 'eas', operation, error: { code: 'eas_project_id_required', message: 'EAS project_id required' } };
      }
      ...
      try {
        ...
        return {
          ...
          project_id: projectId,
          attempt,
          error: { code: 'eas_adapter_error', message: 'EAS adapter execution failed' },
        };
      }
    }
  });
