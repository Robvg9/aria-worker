...
const ALLOWED_COMPUTER_ACTIONS = new Set([
  'screenshot',
  'observe',
  'open',
  'focus',
...
function compileMission({ goal, target = 'windows-local', constraints = {} } = {}) {
  ...
  if (strategy === 'computer' || strategy === 'hybrid') {
    steps.push(action(COMPUTER_OPERATION, { action: 'observe' }));
  }

  if (/powershell|terminal/i.test(normalizedGoal) && strategy !== 'shell') {
    steps.push(action(COMPUTER_OPERATION, { action: 'open', target: 'PowerShell' }));
  }

  if (strategy === 'shell' || strategy === 'hybrid') {
    steps.push(action(COMPUTER_OPERATION, { action: 'open', target: 'PowerShell' }));
    steps.push(action(COMPUTER_OPERATION, { action: 'focus', target: 'PowerShell' }));
  }
...
function validateStep(step) {
  ...
  if (step.type === SHELL_OPERATION) {
    if (!isAllowedShellScript(step.payload?.script)) return { ok: false, reason: 'shell_policy_blocked' };
    return { ok: true };
  }
...
async function executePlan(plan, execute, verify = async () => ({ ok: true })) {
  ...
  for (let i = 0; i < plan.steps.length; i += 1) {
    ...
    if (!verification || verification.ok !== true) {
      return Object.freeze({ status: 'needs_replan', failed_index: i, results, verification });
    }
  }
