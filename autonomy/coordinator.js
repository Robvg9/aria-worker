...
function createAutonomyCoordinator({ policy = {}, execute, verify, now } = {}) {
  ...
  async function run() {
    return runAutonomousLoop({
      ...
      execute: async goal => { resources.recordAction(); try { const result = await execute(goal); return { goal, result }; } catch (error) { resources.recordFailure(); return { goal, error: String(error && error.message || error) }; } },
