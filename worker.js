...
async function proxyAppApi(request,url){
  const suffix=url.pathname.replace(/^\/api(?=\/|$)/,"")||"/";
  const upstreamUrl=SUPABASE_APP_API+suffix+url.search;
...
async function proxyPasswordSignIn(request, url){
  ...
  try{
    ...
    const timedOut=error?.name==="AbortError";
    return json({error:"auth_upstream_unreachable",error_description:timedOut?"No se pudo contactar con el servicio de autenticación a tiempo.":"No se pudo contactar con el servicio de autenticación."},504);
  }finally{clearTimeout(timer);}
}
function authorizationPage(pendingId,origin,clientName="Grok Custom Connector"){const id=encodeURIComponent(String(pendingId));const safeName=escapeHtml(clientName||"Grok Custom Connector");const allow=`${origin}/authorize/consent?pending_id=${id}&decision=allow`;const deny=`${origin}/authorize/consent?pending_id=${id}&decision=deny`;return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>Authorize ARIA MCP</title><style>:root{color-scheme:dark;--bg:#090b12;--card:#111522;--line:#252b3c;--text:#f6f7fb;--muted:#a5adc2;--accent:#8b5cf6;--accent2:#6366f1}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(900px 500px at 50% 0,#1a1634 0%,var(--bg) 58%);font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;color:var(--text);padding:22px}.card{width:min(430px,100%);background:rgba(17,21,34,.96);border:1px solid var(--line);border-radius:22px;box-shadow:0 24px 80px rgba(0,0,0,.45);padding:30px}.logo{width:48px;height:48px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,var(--accent),var(--accent2));font-weight:800;font-size:20px;margin-bottom:22px}h1{font-size:26px;margin:0 0 10px}.sub{color:var(--muted);line-height:1.5;margin:0 0 26px}.btn{display:block;width:100%;margin-top:14px;padding:14px 16px;border-radius:12px;background:linear-gradient(135deg,var(--accent),var(--accent2));color:white;font-weight:700;font-size:16px;text-align:center;text-decoration:none}.btn.secondary{background:#2a3144}.foot{margin-top:18px;font-size:12px;color:#7f879c;text-align:center}</style></head><body><main class="card"><div class="logo">A</div><h1>Authorize ARIA MCP</h1><p class="sub"><strong>${safeName}</strong> is requesting access to ARIA read-only tools.</p><p class="sub">Scope: <code>aria.mcp.inbound</code><br>Resource: ARIA inbound MCP</p><a class="btn" href="${allow}">Authorize</a><a class="btn secondary" href="${deny}">Deny</a><div class="foot">Secure OAuth · ARIA MCP</div></main></body></html>`}
async function oauthFetch(input,init={}){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),10000);try{return await fetch(input,{...init,signal:controller.signal})}catch(error){return null}finally{clearTimeout(timer)}}
async function proxyOAuth(request,url){
  ...
  if(suffix === "authorize/consent" && request.method === "GET"){
    const pendingId=url.searchParams.get("pending_id")||"";const decision=url.searchParams.get("decision")||"";
    if(!pendingId||!['allow','deny'].includes(decision))return new Response("Invalid consent request",{status:400,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}});
    const body=new URLSearchParams({pending_id:pendingId,decision}).toString();
    const upstream=await oauthFetch(upstreamUrl.toString(),{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded","accept":"text/html,application/json"},body,redirect:"manual"});
    if(!upstream)return new Response("OAuth upstream timeout",{status:504,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store","x-aria-oauth-upstream":"timeout"}});
    const h=new Headers(upstream.headers);h.set("cache-control","no-store");return new Response(upstream.body,{status:upstream.status,statusText:upstream.statusText,headers:h});
  }
  upstreamUrl.search=url.search;
  const headers=new Headers(request.headers);headers.delete("host");
  const upstream=await oauthFetch(new Request(upstreamUrl.toString(),{method:request.method,headers,body:request.method==="GET"||request.method==="HEAD"?undefined:request.body,redirect:"manual"}));
  if(!upstream)return new Response("OAuth upstream timeout",{status:504,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store","x-aria-oauth-upstream":"timeout"}});
  if(url.pathname==="/register"||url.pathname==="/register/"){
    const h=new Headers(upstream.headers);h.set("content-type","application/json; charset=utf-8");h.set("cache-control","no-store");const raw=await upstream.text();
    try{const body=JSON.parse(raw);body.authorization_endpoint=PUBLIC_AUTH_ENDPOINT;body.token_endpoint=PUBLIC_TOKEN_ENDPOINT;body.registration_endpoint=PUBLIC_REGISTER_ENDPOINT;body.issuer=PUBLIC_ISSUER;body.resource=PUBLIC_RESOURCE;return new Response(JSON.stringify(body),{status:upstream.status,statusText:upstream.statusText,headers:h})}catch(_){return new Response(raw,{status:upstream.status,statusText:upstream.statusText,headers:h})}
  }
  if((url.pathname==="/authorize"||url.pathname==="/authorize/"||url.pathname==="/authorize-grok-v2"||url.pathname==="/authorize-grok-v2/")&&request.method==="GET"){
    const h=new Headers(upstream.headers);h.set("cache-control","no-store");
    if(upstream.status>=300&&upstream.status<400)return new Response(null,{status:upstream.status,headers:h});
...
export default {async scheduled(_controller,env,ctx){ctx.waitUntil(runScheduledMission(env))},async fetch(request,env){const url=new URL(request.url);
...
if(url.pathname==="/pwa"||url.pathname.startsWith("/pwa/")){if(request.method!=="GET"&&request.method!=="HEAD")return new Response("Method Not Allowed",{status:405});const assetUrl=new URL(request.url);assetUrl.pathname=url.pathname.replace(/^\/pwa(?=\/|$)/,"")||"/";let shellPath=assetUrl.pathname;const oldIndex=/^\/index-[^/]+\.html$/.test(shellPath);const oldManifest=/^\/manifest-[^/]+\.json$/.test(shellPath);const oldWorker=/^\/sw-[^/]+\.js$/.test(shellPath);if(shellPath==="/version.json")return json({ok:true,service:"aria-pwa-version",build:PWA_BUILD,release:PWA_RELEASE,cache:"no-store"});if(shellPath==="/"||shellPath==="/index.html"||oldIndex)shellPath="/index-"+PWA_BUILD+".html";if(shellPath==="/manifest.json"||oldManifest)shellPath="/manifest.json";if(shellPath==="/sw.js"||oldWorker)shellPath="/sw-"+PWA_BUILD+".js";assetUrl.pathname=shellPath;const isServiceWorker=shellPath==="/sw-"+PWA_BUILD+".js";const isShell=shellPath==="/index-"+PWA_BUILD+".html"||shellPath==="/manifest.json"||shellPath==="/sw-"+PWA_BUILD+".js";const assetRequest=new Request(assetUrl.toString(),{method:request.method,headers:request.headers,...(isShell?{cache:"no-store"}:{})});const assetResponse=await env.ARIA_PWA_ASSETS.fetch(assetRequest);const responseHeaders=new Headers(assetResponse.headers);if(isServiceWorker&&assetResponse.ok){const swLines=(await assetResponse.text()).split("\n");swLines[0] = "const CACHE = " + JSON.stringify("aria-pwa-" + PWA_BUILD) + ";";responseHeaders.set("content-type","application/javascript; charset=utf-8");responseHeaders.set("cache-control","no-store, max-age=0");responseHeaders.set("cdn-cache-control","no-store");responseHeaders.set("x-aria-pwa-build",PWA_BUILD);if(request.method==="HEAD")return new Response(null,{status:assetResponse.status,statusText:assetResponse.statusText,headers:responseHeaders});return new Response(swLines.join("\n"),{status:assetResponse.status,statusText:assetResponse.statusText,headers:responseHeaders});}if(isShell){responseHeaders.set("cache-control","no-store, max-age=0");responseHeaders.set("cdn-cache-control","no-store");responseHeaders.set("x-aria-pwa-build",PWA_BUILD);}return new Response(assetResponse.body,{status:assetResponse.status,statusText:assetResponse.statusText,headers:responseHeaders})}if(url.pathname==="/api"||url.pathname.startsWith("/api/"))return proxyAppApi(request,url);if(url.pathname==="/auth/token")return proxyPasswordSignIn(request,url);if(url.pathname==="/autonomy-health")return autonomyHealth(request);if(request.method==="GET"&&(url.pathname==="/.well-known/oauth-protected-resource"||url.pathname==="/.well-known/oauth-protected-resource/mcp"||url.pathname==="/mcp/.well-known/oauth-protected-resource"))return json(protectedResourceMetadata(),200,{"access-control-allow-origin":"*"});if(request.method==="GET"&&(url.pathname==="/.well-known/oauth-authorization-server"||url.pathname==="/.well-known/oauth-authorization-server/functions/v1/aria-mcp-oauth-grok-v2"||url.pathname==="/.well-known/oauth-authorization-server/functions/v1/aria-mcp-oauth-grok-v3"||url.pathname==="/.well-known/oauth-authorization-server/functions/v1/aria-mcp-oauth-grok-v4"||url.pathname==="/.well-known/oauth-authorization-server/functions/v1/aria-mcp-inbound-grok-v1"))return json(authorizationServerMetadata(),200,{"access-control-allow-origin":"*"});if(url.pathname==="/authorize"||url.pathname==="/authorize/"||url.pathname.startsWith("/authorize/")||url.pathname==="/authorize-grok-v2"||url.pathname==="/authorize-grok-v2/"||url.pathname.startsWith("/authorize-grok-v2/"))return proxyOAuth(request,url);if(["/token","/token/","/register","/register/"].includes(url.pathname))return proxyOAuth(request,url);if(url.pathname==="/aria"||url.pathname==="/aria/")return directAria(request,env);if(url.pathname==="/mission"||url.pathname==="/mission/")return startMission(request,env);if(url.pathname==="/runtime"||url.pathname==="/runtime/")return proxyRuntime(request,env);if(url.pathname==="/admin/cloudflare"||url.pathname==="/admin/cloudflare/")return cloudflareAdmin(request,env);if(url.pathname==="/admin/cloudflare/token"||url.pathname==="/admin/cloudflare/token/")return cloudflareTokenManager(request,env);if(url.pathname==="/mcp"||url.pathname==="/mcp/"){const upstreamUrl=new URL(SUPABASE_MCP);upstreamUrl.search=url.search;const headers=new Headers(request.headers);const upstream=await fetch(new Request(upstreamUrl.toString(),{method:request.method,headers,body:request.method==="GET"||request.method==="HEAD"?undefined:request.body,redirect:"manual"}));if(upstream.status===401)return rewriteAuthChallenge(upstream);return upstream}if(url.pathname==="/"||url.pathname==="")return Response.redirect(`${url.origin}/mcp`,308);return new Response("Not Found",{status:404,headers:{"content-type":"text/plain; charset=utf-8"}})}};
