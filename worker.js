...
function protectedResourceMetadata() { return { resource: RESOURCE, authorization_servers: [SUPABASE_OAUTH], bearer_methods_supported: ["header"], scopes_supported: SCOPES }; }
function authorizationServerMetadata() { return { issuer: SUPABASE_OAUTH, authorization_endpoint: `${SUPABASE_OAUTH}/authorize`, token_endpoint: `${SUPABASE_OAUTH}/token`, registration_endpoint: `${SUPABASE_OAUTH}/register`, response_types_supported: ["code"], grant_types_supported: ["authorization_code"], code_challenge_methods_supported: ["S256"], token_endpoint_auth_methods_supported: ["none"], scopes_supported: SCOPES }; }
function constantTimeEqual(a, b) { if (typeof a !== "string" || typeof b !== "string" || a.length !== b.length) return false; let result = 0; for (let i = 0; i < a.length; i++) result |= a.charCodeAt(i) ^ b.charCodeAt(i); return result === 0; }
...
async function runScheduledMission(env) {
  ...
  if (!serviceKey) { console.error("[ARIA CRON] scheduler service authority not configured"); return; }
  try {
    const response = await fetch(RUNNER_URL, { method: "POST", headers: { "content-type": "application/json", "authorization": `Bearer ${serviceKey}` }, body: "{}" });
    console.log(`[ARIA CRON] runner status=${response.status}`);
    if (!response.ok) { const text = await response.text().catch(() => ""); console.error(`[ARIA CRON] runner failure status=${response.status} body=${text.slice(0, 500)}`); }
  } catch (error) { console.error(`[ARIA CRON] runner request failed: ${error instanceof Error ? error.message : String(error)}`); }
}
export default {
  async scheduled(_controller, env, ctx) { ctx.waitUntil(runScheduledMission(env)); },
  async fetch(request, env) {
    const url = new URL(request.url);
    if (request.method === "GET" && (url.pathname === "/.well-known/oauth-protected-resource" || url.pathname === "/.well-known/oauth-protected-resource/mcp")) return json(protectedResourceMetadata(), 200, { "access-control-allow-origin": "*" });
    if (request.method === "GET" && (url.pathname === "/.well-known/oauth-authorization-server" || url.pathname === "/.well-known/oauth-authorization-server/functions/v1/aria-mcp-oauth-grok-v2")) return json(authorizationServerMetadata(), 200, { "access-control-allow-origin": "*" });
