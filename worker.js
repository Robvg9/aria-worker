...
async function proxyOAuth(request,url){
  ...
  // Rewrite resource/redirect hints for public Worker URLs when needed
  if ((suffix === "authorize" || suffix === "token" || suffix === "register") && request.method !== "GET") {
    // body handled below
  }
  ...
  if(url.pathname==="/register"||url.pathname==="/register/"){
    const responseHeaders=new Headers(upstream.headers);responseHeaders.set("content-type","application/json; charset=utf-8");responseHeaders.set("cache-control","no-store");
    const raw=await upstream.text();
    try{const body=JSON.parse(raw);body.authorization_endpoint=PUBLIC_AUTH_ENDPOINT;body.token_endpoint=PUBLIC_TOKEN_ENDPOINT;body.registration_endpoint=PUBLIC_REGISTER_ENDPOINT;body.issuer=PUBLIC_ISSUER;body.resource=PUBLIC_RESOURCE;return new Response(JSON.stringify(body),{status:upstream.status,statusText:upstream.statusText,headers:responseHeaders})}catch(_){return new Response(raw,{status:upstream.status,statusText:upstream.statusText,headers:responseHeaders})}
  }

  // For authorize GET: if upstream returns HTML consent, optionally wrap; otherwise pass through redirects
  if((url.pathname==="/authorize"||url.pathname==="/authorize/"||url.pathname==="/authorize-grok-v2"||url.pathname==="/authorize-grok-v2/") && request.method==="GET"){
    ...
    // Pass through redirects (302) as-is for OAuth
    if(upstream.status>=300 && upstream.status<400){
      return new Response(null,{status:upstream.status,headers:responseHeaders});
    }
    responseHeaders.set("content-type","text/html; charset=utf-8");
    const html=await upstream.text();
    // If inbound already rendered consent page, return it (rewrite absolute ISSUER links is not needed if we set RESOURCE env)
