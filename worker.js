...
async function proxyRuntime(request, env) {
  ...
  const upstream = await fetch(RUNTIME_GATEWAY, { method: "POST", headers: { "content-type": "application/json", "authorization": `Bearer ${env.ARIA_RUNTIME_SHARED_SECRET}` }, body });
...
async function runScheduledMission(env) {
  if (!env.ARIA_RUNTIME_SHARED_SECRET) return;
  try {
    const response = await fetch(RUNNER_URL, { method: "POST", headers: { "content-type": "application/json", "authorization": `Bearer ${env.ARIA_RUNTIME_SHARED_SECRET}` }, body: "{}" });
    if (!response.ok) console.error(`[ARIA CRON] runner failure status=${response.status}`);
  } catch (error) {
    console.error(`[ARIA CRON] runner request failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
export default {
  async scheduled(controller, env, ctx) { ctx.waitUntil(runScheduledMission(env)); },
  async fetch(request, env) {
    ...
    if (url.pathname === "/mcp" || url.pathname === "/mcp/") {
      ...
      const incomingTrace = headers.get("X-ARIA-Trace-Id"); if (incomingTrace) headers.set("X-ARIA-Trace-Id", incomingTrace);
