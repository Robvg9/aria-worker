...
async function run() {
  ...
  const result = await adapter.issue({ credential_id: 'worker.read', template: 'worker_read', name: 'ARIA worker read' });
  ...
  assert.deepEqual(JSON.parse(calls[0].init.body), { template: 'worker_read', name: 'ARIA worker read' });
  ...
  await assert.rejects(
    () => adapter.issue({ credential_id: 'worker/read', template: 'worker_read' }),
    /credential_id_invalid/
  );

  await assert.rejects(
    () => adapter.issue({ credential_id: 'worker.read', template: 'unsupported' }),
    /unsupported_token_template/
  );
