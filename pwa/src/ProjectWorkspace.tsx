...
function VisualBoard({session,project,onChat,onMission}:{session:Session;project:Project;onChat:(message:string)=>void;onMission:(payload:any)=>void}) {
...
export function ProjectWorkspace({session,onBack}:{session:Session;onBack:()=>void}) {
  ...
  useEffect(()=>{void loadMissions();setMessages([])},[project.id]);
  ...
  return <main className='appShell projectShell'>
    ...
    {tab==='visual'&&<VisualBoard session={session} project={project} onChat={m=>{setMessages(x=>[...x,{id:crypto.randomUUID(),role:'aria',text:m}]);setTab('chat')}} onMission={createMission}/>}
