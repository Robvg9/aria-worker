...
function upsertEntity(entities = [], entity) {
  ...
  const out = [...entities]; out[i] = { ...out[i], ...normalized }; return out;
}
...
function activeAt(item, at = new Date().toISOString()) {
  const t = Date.parse(at); const start = item.valid_from ? Date.parse(item.valid_from) : -Infinity; const end = item.valid_until ? Date.parse(item.valid_until) : Infinity;
  return t >= start && t < end;
}
function contradictions(items = []) {
  const groups = new Map();
  for (const x of items) { const k = String(x.subject || x.title || '').toLowerCase(); if (!k) continue; (groups.get(k) || groups.set(k, []).get(k)).push(x); }
  return [...groups.entries()].flatMap(([subject, xs]) => { const values = new Set(xs.map(x => String(x.content ?? ''))); return values.size > 1 ? [{ subject, items: xs }] : []; });
}
function attachProvenance(item, provenance = {}) {
  const source = provenance.source || provenance.source_id; if (!source) throw new Error('provenance_source_required');
...
function consolidateEvidence(items = []) {
  const valid = items.filter(item => item && (item.provenance?.source || item.provenance?.source_id)); const byValue = new Map();
  for (const item of valid) { const key = `${item.subject || item.title || ''}|${item.content || ''}`.toLowerCase(); const current = byValue.get(key); if (!current || Number(item.confidence ?? 0) > Number(current.confidence ?? 0)) byValue.set(key, item); }
  return [...byValue.values()].sort((a, b) => Number(b.confidence ?? 0) - Number(a.confidence ?? 0));
}
function repairContradictions(items = []) {
  return contradictions(items).map(group => { const ranked = [...group.items].sort((a, b) => Number(b.confidence ?? 0) - Number(a.confidence ?? 0)); return Object.freeze({ subject: group.subject, winner: ranked[0] || null, rejected: ranked.slice(1), repaired: ranked.length > 0 }); });
}
