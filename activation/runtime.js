...
const { envNameForRef, createEnvironmentSecretResolver } = require('./secrets');
...
function createActivationRuntime({ manifest = DEFAULT_MANIFEST, env = process.env, fetchImpl = globalThis.fetch, authorize = null } = {}) {
  ...
  async function probeAll() { const results=[]; for (const entry of entries) results.push(await probe(entry)); return results; }
  ...
  async function execute(id, operation, context = {}) {
    ...
    const declaredRisk = adapter.descriptor.operation_risk && adapter.descriptor.operation_risk[operation];
    if (!declaredRisk || !RISK_CLASSES.includes(declaredRisk)) return { status:'blocked', reason:'operation_risk_undefined' };
    const requestedRisk = context.risk_class || declaredRisk;
  ...
  function snapshot() { return entries.map(entry => ({ connector_id:entry.connector_id, enabled:entry.enabled, state:status(entry.connector_id), required:entry.required, credential_configured: entry.credential_ref ? Boolean(env && env[envNameForRef(entry.credential_ref)]) : true })); }
