...
const adapters = {
  github: createAdapter({connector_id:'github', operations:['repo_read','file_read','file_write','branch_create','pull_request','workflow_dispatch'], operation_risk:{repo_read:'READ',file_read:'READ',file_write:'LOW_RISK_WRITE',branch_create:'LOW_RISK_WRITE',pull_request:'LOW_RISK_WRITE',workflow_dispatch:'HIGH_RISK_WRITE'}}, async (op,c) => {
    const base=c.base_url||'https://api.github.com'; const h=authHeaders(c.secret,'github'); const owner=encodeURIComponent(c.owner||''); const repo=encodeURIComponent(c.repo||'');
    if(op==='health') return request({url:`${base}/rate_limit`,headers:h,fetchImpl:c.fetchImpl});
    if(op==='repo_read') return request({url:`${base}/repos/${owner}/${repo}`,headers:h,fetchImpl:c.fetchImpl});
    if(op==='file_read') return request({url:`${base}/repos/${owner}/${repo}/contents/${c.path}`,headers:h,fetchImpl:c.fetchImpl});
    if(op==='file_write') return request({url:`${base}/repos/${owner}/${repo}/contents/${c.path}`,method:'PUT',headers:{...h,'Content-Type':'application/json'},body:{message:c.message,content:Buffer.from(String(c.content||''),'utf8').toString('base64'),sha:c.sha,branch:c.branch,committer:c.committer,author:c.author},fetchImpl:c.fetchImpl});
    if(op==='branch_create') return request({url:`${base}/repos/${owner}/${repo}/git/refs`,method:'POST',headers:{...h,'Content-Type':'application/json'},body:{ref:`refs/heads/${c.branch}`,sha:c.sha},fetchImpl:c.fetchImpl});
    if(op==='pull_request') return request({url:`${base}/repos/${owner}/${repo}/pulls`,method:'POST',headers:{...h,'Content-Type':'application/json'},body:{title:c.title,head:c.head,base:c.base,body:c.body,draft:c.draft,maintainer_can_modify:c.maintainer_can_modify},fetchImpl:c.fetchImpl});
    if(op==='workflow_dispatch') return request({url:`${base}/repos/${owner}/${repo}/actions/workflows/${c.workflow_id}/dispatches`,method:'POST',headers:{...h,'Content-Type':'application/json'},body:{ref:c.ref||'main',inputs:c.inputs||{}},fetchImpl:c.fetchImpl});
  ...
  supabase: createAdapter({connector_id:'supabase', operations:['project_read','db_read','migration','logs','edge_function'], operation_risk:{project_read:'READ',db_read:'READ',migration:'HIGH_RISK_WRITE',logs:'READ',edge_function:'HIGH_RISK_WRITE'}}, async (op,c) => {
    ...
    if(op==='edge_function') return request({url:`${base}/v1/projects/${ref}/functions/deploy${c.slug?`?slug=${encodeURIComponent(c.slug)}`:''}`,method:'POST',headers:{...h},rawBody:c.form_data,fetchImpl:c.fetchImpl});
    return {ok:false,status:400,data:{error:'unsupported_operation'}};
  }),
  cloudflare: createAdapter({connector_id:'cloudflare', operations:['account_read','worker_read','deployments_read','deployment_create','worker_write','logs'], operation_risk:{account_read:'READ',worker_read:'READ',deployments_read:'READ',deployment_create:'HIGH_RISK_WRITE',worker_write:'HIGH_RISK_WRITE',logs:'READ'}}, async (op,c) => {
    ...
    if(op==='worker_write') return request({url:`${base}/accounts/${account}/workers/scripts/${script}/content`,method:'PUT',headers:{...h,'Content-Type':c.content_type||'application/javascript'},rawBody:String(c.content||''),fetchImpl:c.fetchImpl});
