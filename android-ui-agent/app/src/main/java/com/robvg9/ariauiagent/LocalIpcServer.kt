...
class LocalIpcServer(private val service: AriaAccessibilityService) {
    ...
    private fun handleClient(socket: Socket) {
        socket.use {
            try {
                ...
                if (matched != 4) { writeResponse(socket.getOutputStream(), 400, """{"ok":false,"reason":"headers_invalid"}"""); return }
                ...
                if (method != "POST" || path != IpcAuth.PATH) { writeResponse(socket.getOutputStream(), 404, """{"ok":false,"reason":"route_not_found"}"""); return }
                if (headers["authorization"] != "Bearer " + IpcAuth.TOKEN) { writeResponse(socket.getOutputStream(), 401, """{"ok":false,"reason":"unauthorized"}"""); return }
                val length = headers["content-length"]?.toIntOrNull()
                if (length == null || length <= 0 || length > 262144) { writeResponse(socket.getOutputStream(), 413, """{"ok":false,"reason":"body_length_invalid"}"""); return }
                val body = ByteArray(length)
                var offset = 0
                while (offset < length) {
                    val count = input.read(body, offset, length - offset)
                    if (count <= 0) { writeResponse(socket.getOutputStream(), 400, """{"ok":false,"reason":"body_incomplete"}"""); return }
                    offset += count
                }
                val result = service.handle(String(body, StandardCharsets.UTF_8))
                writeResponse(socket.getOutputStream(), 200, result.toString())
            } catch (_: Exception) {
                try { writeResponse(socket.getOutputStream(), 500, """{"ok":false,"reason":"ipc_server_error"}""") } catch (_: Exception) {}
            }
        }
    }
    ...
    private fun writeResponse(output: OutputStream, status: Int, body: String) {
        ...
        val reason = when (status) { 200 -> "OK"; 400 -> "Bad Request"; 401 -> "Unauthorized"; 404 -> "Not Found"; 413 -> "Payload Too Large"; else -> "Internal Server Error" }
