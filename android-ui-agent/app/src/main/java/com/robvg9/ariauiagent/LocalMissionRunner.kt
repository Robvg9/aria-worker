...
class LocalMissionRunner(
    ...
    fun approveAndExecute(): LocalMission = synchronized(lock) {
        ...
        val observedPkg = BrowserHandoff.selectObservedBrowserPackage(moving.steps)
            ?: return failWithDiag(
                ...
                JSONObject()
                    .put("resolvePhase", "browser_handoff")
        ...
        val handoffRaw = try {
            ...
            return failWithDiag(
                ...
                JSONObject()
                    .put("resolvePhase", "browser_handoff")
                    .put("observedBrowserPackage", observedPkg)
                    .put("startActivityError", e.message?.take(80))
            )
        }
        if (!handoffRaw.optBoolean("ok", false)) {
            val hDiag = handoffRaw.optJSONObject("diagnostic") ?: JSONObject()
            hDiag.put("resolvePhase", "browser_handoff")
            return failWithDiag(
                handoffRaw.optString("reason", "browser_handoff_failed"),
                hDiag
            )
        }
    ...
    private fun failWithDiag(reason: String, diagnostic: JSONObject): LocalMission {
        ...
        val obs = LocalObservation(
            packageName = diagnostic.optString("selectedPackage", null)
                ?: diagnostic.optString("observedBrowserPackage", null),
            ok = false,
            error = reason,
            diagnosticJson = diagnostic.toString()
        )
