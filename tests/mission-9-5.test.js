...
mustContain(mcp, 'methodEquals(mcpMethod, "tools/call")', 'MCP tools call path');
mustContain(mcp, 'auth_required', 'MCP auth gate');
assert.ok(/methodEquals\(mcpMethod, "initialize"\)[\s\S]*methodEquals\(mcpMethod, "tools\/list"\)/.test(mcp), 'initialize/tools-list share provisioning guard');
assert.ok(/methodEquals\(mcpMethod, "tools\/call"\)[\s\S]*authUser\(req\)/.test(mcp), 'tools/call remains authenticated');
...
console.log('PASS: Mission 9.5 Grok OAuth + safe E2E trace + verified HTML UI origin + explicit tool OAuth security metadata');
