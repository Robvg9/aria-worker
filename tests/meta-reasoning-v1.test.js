...
function evidence(overrides={}) {
  return [
    ...
    {source_id:'s2', observed:true, verified:true, relevant:true, ...overrides}
  ];
}
...
assert.equal(r.evidence_sufficient, true);
...
r = assess({task:'retry failed strategy', confidence:0.95, evidence:evidence({}), current_strategy:'same', strategy_history:[{strategy:'same',outcome:'failed'},{strategy:'same',outcome:'failed'}], skills:[{skill_id:'better-skill',available:true,fit_score:0.92}]});
...
r = assess({task:'ambiguous action', confidence:0.9, evidence:evidence({}), ambiguity:0.8});
assert.equal(r.status, ACTIONS.INVESTIGATE);
...
r = assess({task:'contradictory evidence', confidence:0.95, evidence:[{source_id:'s1',observed:true,verified:true,relevant:true},{source_id:'s2',observed:true,verified:true,relevant:true,contradiction:true}]});
assert.equal(r.status, ACTIONS.ABSTAIN);
assert.equal(r.uncertainty.includes('evidence_conflict'), true);
