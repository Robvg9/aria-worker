...
commands=['PAUSA'];await controller.tick('command');assert.equal(controller.status().mode,'paused');commands=['AVANZA'];await controller.tick('command');assert.equal(controller.status().mode,'active');commands=['ESTADO'];await controller.tick('command');assert.ok(events.some(x=>x.includes('STATUS')));commands=['CERRAR'];await controller.tick('command');assert.equal(controller.status().mode,'stopped');assert.ok(checkpoints>=1);await controller.shutdown();console.log('MEDITATION_IA_CORE_TEST=PASS');console.log(JSON.stringify({version:controller.version,ticks,checkpoints,events:events.length}));})().catch(e=>{console.error(e);process.exit(1)});
