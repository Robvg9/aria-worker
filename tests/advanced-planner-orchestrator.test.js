...
test('advanced planner integrates with autonomous orchestrator: failure -> replan -> success', async () => {
  ...
  const mission = { mission_id: 'advanced-orchestrator-e2e', status: 'queued', goal: 'orchestrated advanced replan', current_step: 0, completed_steps: 0, checkpoint: {} };
  const store = {
    async get() { return mission; },
    async transition(_id, status, patch = {}) { mission.status = status; Object.assign(mission, patch); return mission; },
    async checkpoint(_id, checkpoint, patch = {}) { mission.checkpoint = checkpoint; Object.assign(mission, patch); return mission; },
  };
  let executions = 0;
  const orchestrator = createAutonomousMissionOrchestrator({
    missionStore: store,
    planner: async () => { plannerState = runtime.start(planned); return plannerState.selected_plan; },
    replanner: async ({ outcome }) => { plannerState = runtime.replan(plannerState, { reason: outcome?.error || 'executor_failed' }); return plannerState.selected_plan; },
    executor: async ({ step }) => { executions += 1; return step.id === 'fallback_1' ? { status: 'succeeded', executor_type: 'test' } : { status: 'failed', executor_type: 'test' }; },
