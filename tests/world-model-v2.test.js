...
assert.equal(snap.entities.length,6); assert.equal(snap.relationships.length,4); assert.equal(snap.dependencies.length,4);
...
assert.equal(wm.dependentsOf('fn_x').length,3); assert.equal(wm.dependenciesOf('fn_x').length,1);
const explanation = wm.explainChange({entity_id:'fn_x',change:{state:'degraded'}});
assert.equal(explanation.entity.id,'fn_x'); assert.equal(explanation.affected_entities.length,3); assert.equal(explanation.prerequisite_entities.length,1); assert.equal(explanation.prerequisite_entities[0].id,'secret_z');
const observed = wm.observeState('fn_x','degraded',p('live-probe'));
assert.equal(observed.event.type,'state_observed'); assert.deepEqual(observed.event.changes,{from:'healthy',to:'degraded'}); assert.equal(wm.stateEvidence('fn_x').current_state,'degraded'); assert.equal(wm.stateEvidence('fn_x').observations.length,1);
...
assert.equal(wm.causeChain('evt_worker')[0].event_id,'evt_root'); assert.equal(wm.causeChain('evt_root')[0].event_id,'evt_prior');
...
const bad = createWorldModelV2({entities:[{id:'a',type:'x',provenance:p('seed')}],dependencies:[{from:'a',to:'missing',type:'depends_on',provenance:p('bad')}]});
assert.equal(bad.integrity().valid,false); assert.deepEqual(bad.integrity().missing_refs,['missing']);
const before = wm.snapshot(); const again = wm.impactOf('fn_x');
assert.deepEqual(again.impacted.map(x=>x.entity_id),impact.impacted.map(x=>x.entity_id)); assert.deepEqual(wm.snapshot().entities.map(e=>e.id),before.entities.map(e=>e.id));
assert.equal(wm.integrity().valid,true); assert.equal(JSON.stringify(wm.snapshot()).includes('super-secret-value'),false);
const noTime = JSON.stringify(require('../memory/world-model-v2').createWorldModelV2({entities:[{id:'a',type:'x',provenance:p('seed')}]}).snapshot());
assert.equal(noTime.includes('generated_at'),false);
