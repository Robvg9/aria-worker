...
async function main() {
  ...
  // ── TEST 11: execution_id determinista/único
  {
    ...
    eq(engine.canonicalJson({ b: 1, a: [2, { d: 1, c: 2 }] }), engine.canonicalJson({ a: [2, { c: 2, d: 1 }], b: 1 }), 'TEST 11e: canonical json is key-order independent');
  ...
  // ── TEST 13: no modification of Router / Fallback
  {
    ...
    const src = fs.readFileSync(path.join(__dirname, '..', 'execution', 'lookup.js'), 'utf8');
    ok(/require\('\.\.\/fallback\/lookup\.js'\)/.test(src) && /candidateSelectable/.test(src), 'TEST 13h: engine consumes 10.7 candidateSelectable (no reimplementation)');
    ok(!/collectCandidates|compareCandidates|applyPreferences|preferred_/.test(src), 'TEST 13i: engine has no candidate enumeration/preference logic (no routing)');
  }
  ...
  // ── TEST 14: no exposición de secretos
  {
    ...
    ok(w.calls.transport[0].opts.headers.Authorization.indexOf(FAKE_SECRET) > 0, 'TEST 14d: secret only reaches transport headers (mock)');
  ...
  // ── TEST 16: no bypass de quota/capacity (unknown ≠ available)
  {
    ...
    // Direct Gemini is now an explicitly verified route. The local 10.8 engine
    // deliberately has no production credential resolver, so it must fail closed
    // at credential resolution rather than inventing no_route.
    ...
    eq(router.route({ capability: CAP }).status, 'selected', 'TEST 16d: router selects verified direct Gemini');
    eq(viaRouter.status, 'failed', 'TEST 16e: selected direct Gemini route fails closed without production credential resolver');
    eq(viaRouter.error.code, 'credential_unavailable', 'TEST 16f: credential boundary blocks local execution without secret');
    eq(viaRouter.metadata.mode, 'mock', 'TEST 16g: local test remains mock mode');
  }

  // ── TEST 17: default deps / LIVE guard
  {
    const w = world();
    const r = await engine.execute({ selected_route: selected(A), authorization: APPROVED, input: INPUT }, { ...w.deps, credentialResolver: undefined });
  ...
  // ── TEST 18: adapter abstraction (10.13)
  {
    ...
    ok(d.adapter_id && d.provider_id === 'openrouter' && d.interface_type && Array.isArray(d.operations), 'TEST 18: adapter declares id/provider/interface/operations');
    ...
    const w = world({ models: { m2: { model_id: 'm2', provider_id: 'openrouter', status: 'available' } }, supports: { ['m2|' + CAP]: true }, quota: { [A.account_id + '|m2']: 'available' } });
  ...
  // ── TEST 19: no memory writers / no telemetry persistence
  {
    ...
    for (const f of files) {
      const txt = fs.readFileSync(path.join(__dirname, '..', f), 'utf8');
      ok(!/supabase|notion\.|notion-|api\.notion|writeFile|appendFile|localStorage|require\('fs'\)|from\('.*supabase/i.test(txt), `TEST 19: ${f} has no memory/persistence writer`);
  ...
  // ── TEST 20: version & contract
  {
    ...
    for (const s of ['succeeded', 'failed', 'blocked', 'cancelled', 'insufficient_evidence', 'CREDENTIAL RESOLVER NOT IMPLEMENTED', 'CAPTURE → GATE → COMMIT → SYNC']) ok(contract.indexOf(s) !== -1, `TEST 20c: contract documents \"${s}\"`);
    for (const s of ['succeeded', 'failed', 'blocked']) ok(execReg.emitted_status.indexOf(s) !== -1, `TEST 20d: registry emits ${s}`);
    ok(execReg.canonical_status.indexOf('cancelled') !== -1 && execReg.emitted_status.indexOf('cancelled') === -1, 'TEST 20e: cancelled reserved (10.13) but not emitted');
  }
