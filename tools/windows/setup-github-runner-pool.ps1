...
if (-not $p.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'PowerShell must run as Administrator.' }
if ($Count -lt 1 -or $Count -gt 8) { throw 'Count must be between 1 and 8.' }
if ([string]::IsNullOrWhiteSpace($RegistrationToken)) { throw 'Set GITHUB_RUNNER_TOKEN or pass -RegistrationToken with a fresh repository runner registration token.' }
...
if (-not (Test-Path $zip)) { Invoke-WebRequest -Uri $asset.browser_download_url -OutFile $zip -UseBasicParsing }
...
for ($i = 1; $i -le $Count; $i++) {
  ...
  if (-not (Test-Path (Join-Path $dir '.runner'))) {
    if (-not (Test-Path (Join-Path $dir 'config.cmd'))) { Expand-Archive -Path $zip -DestinationPath $dir -Force }
  ...
  foreach ($svc in $services) { if ($svc.Status -ne 'Running') { Start-Service -Name $svc.Name } }
