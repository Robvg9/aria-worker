...
function createSelfDevelopmentEngine({ snapshot, rules = [], workspace, testRunner, writer, snapshotStore = null, policy = {} } = {}) {
  ...
  async function improve({ objective = 'self_improvement', proposed_changes = [], scope = null } = {}) {
    ...
    const before = {};
    for (const change of actionable) before[change.path] = await workspace.read(change.path);
    const applied = [];
    for (const change of actionable) {
      const result = await workspace.apply(change);
      if (!result || result.status !== 'succeeded') return { status: 'failed', stage: 'change', applied, failed_path: change.path, inspection, diagnosis, plan };
    ...
    if (!verification.verified) return { status: 'failed', stage: 'verification', applied, before, tests, verification };
