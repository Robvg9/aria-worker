...
function Invoke-Desktop($payload) {
  ...
  switch ($action) {
    ...
    'open' {
      $p = [string]$payload.path; if ([string]::IsNullOrWhiteSpace($p)) { throw 'desktop_open_path_required' }
      $proc = Start-Process -FilePath $p -PassThru; return @{status='succeeded'; action='open'; pid=$proc.Id; path=$p}
    }
    'focus' {
      $name = [string]$payload.process; if ([string]::IsNullOrWhiteSpace($name)) { throw 'desktop_focus_process_required' }
...
'@
      [AriaWindow]::ShowWindowAsync($proc.MainWindowHandle,9) | Out-Null; [AriaWindow]::SetForegroundWindow($proc.MainWindowHandle) | Out-Null
...
'@
    ...
    'type' {
      $text=[string]$payload.text; if ([string]::IsNullOrEmpty($text)) { throw 'desktop_type_text_required' }; if ($text.Length -gt 32768) { throw 'desktop_type_text_too_large' }
      Add-Type @'
using System; using System.Runtime.InteropServices;
public static class AriaKeyboard { [StructLayout(LayoutKind.Sequential)] public struct INPUT { public uint type; public InputUnion U; } [StructLayout(LayoutKind.Explicit)] public struct InputUnion { [FieldOffset(0)] public KEYBDINPUT ki; } [StructLayout(LayoutKind.Sequential)] public struct KEYBDINPUT { public ushort wVk; public ushort wScan; public uint dwFlags; public uint time; public UIntPtr dwExtraInfo; } [DllImport("user32.dll",SetLastError=true)] public static extern uint SendInput(uint nInputs,INPUT[] pInputs,int cbSize); public const uint INPUT_KEYBOARD=1,KEYEVENTF_UNICODE=0x0004,KEYEVENTF_KEYUP=0x0002; }
'@
      $inputs=New-Object 'AriaKeyboard+INPUT[]' ($text.Length*2); $i=0
      foreach($ch in $text.ToCharArray()){ $inputs[$i].type=[AriaKeyboard]::INPUT_KEYBOARD; $inputs[$i].U.ki=[AriaKeyboard]::KEYBDINPUT::new(); $inputs[$i].U.ki.wScan=[int][char]$ch; $inputs[$i].U.ki.dwFlags=[AriaKeyboard]::KEYEVENTF_UNICODE; $i++; $inputs[$i].type=[AriaKeyboard]::INPUT_KEYBOARD; $inputs[$i].U.ki=[AriaKeyboard]::KEYBDINPUT::new(); $inputs[$i].U.ki.wScan=[int][char]$ch; $inputs[$i].U.ki.dwFlags=[AriaKeyboard]::KEYEVENTF_UNICODE -bor [AriaKeyboard]::KEYEVENTF_KEYUP; $i++ }
    ...
    'keypress' {
      $key=[string]$payload.key; $map=@{ENTER=0x0D;ESC=0x1B;TAB=0x09;SPACE=0x20;BACKSPACE=0x08;DELETE=0x2E;HOME=0x24;END=0x23;LEFT=0x25;UP=0x26;RIGHT=0x27;DOWN=0x28;F1=0x70;F2=0x71;F3=0x72;F4=0x73;F5=0x74;F6=0x75;F7=0x76;F8=0x77;F9=0x78;F10=0x79;F11=0x7A;F12=0x7B}
