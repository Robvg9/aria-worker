...
const VERSION = 'aria-windows-desktop-v1.4';
...
function Invoke-Desktop($p) {
  ...
  switch ($action) {
    'screenshot' {
      ...
      if ($width -le 0 -or $height -le 0) { throw "desktop_invalid_virtual_screen:${left},${top},${width},${height}" }
      if ($width -gt 32768 -or $height -gt 32768) { throw "desktop_virtual_screen_too_large:${width}x${height}" }
      $bmp = New-Object -TypeName System.Drawing.Bitmap -ArgumentList ([int]$width),([int]$height)
...
'@
    ...
    'keypress' {
      ...
      Add-Type @' using System; using System.Runtime.InteropServices; public static class AriaKey { [DllImport("user32.dll")] public static extern void keybd_event(byte bVk,byte bScan,uint dwFlags,UIntPtr dwExtraInfo); public const uint KEYUP=0x0002; } '@
    ...
    'scroll' {
      ...
      Add-Type @' using System; using System.Runtime.InteropServices; public static class AriaScroll { [DllImport("user32.dll")] public static extern void mouse_event(uint flags,uint dx,uint dy,uint data,UIntPtr extra); public const uint WHEEL=0x0800; } '@
